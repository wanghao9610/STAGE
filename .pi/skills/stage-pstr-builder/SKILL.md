...
5. Present the whole cut — takeaway, claims in, claims out, figures, layout — and ask before writing via `stage_questionnaire` (plain text when it is unavailable), per §7. Then write `POSTER_PLAN.md`: frontmatter (`cycle:`, `size:`, `layout:`, `state: planned`, `updated:` with a real date per §4, plus `model_id:` and `model_trail:` per §8), the takeaway, the zone table `| Zone | Content | Source | Status |`, and the excluded list with reasons.
