...
通过 `stage_questionnaire` 提问，一次一个，只问探测定不下来的事：哪个候选是主文件、哪些目录是正文 / 证据 / 垃圾、是否存在配对的 STAR 仓库（它来填 `STAR_HOME`）、以及没有任何东西点名 venue 时的目标 venue。选项来自探测结果，并标出推荐。这个点通过之前什么都不写。
...
4. 批准：一个 `stage_questionnaire`——全部批准、按组批准（章节 / 图 / 表 / bib / 样式）、或者中止。未获批准的行不动。
...
- 两个确认点都走 `stage_questionnaire`，一次调用一个问题。它不可用时（无人值守 / 脚本化），回落到纯文本——依然一次一个，依然要求在任何写入之前给出明确答复。
