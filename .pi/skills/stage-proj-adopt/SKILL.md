...
4. Approval: one question via `stage_questionnaire` — approve all, approve by group (sections / figures / tables / bibs / styles), or abort. Unapproved rows do not move.
...
- Both confirmation points go through `stage_questionnaire`, one question per call. If it is unavailable (headless / scripted), fall back to plain text — still one at a time, still requiring an explicit answer before any write.
