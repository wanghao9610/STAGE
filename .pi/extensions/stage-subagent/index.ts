 ...
 *    model in a STAGE record that no session ran on (conventions §8, the model_id /
 *    model_trail fields).
...
function formatToolCall(
	...
	const shortenPath = (p: string) => {
		...
		return p.stagetsWith(home) ? `~${p.slice(home.length)}` : p;
	};
...
function getPiInvocation(args: string[]): { command: string; args: string[] } {
	...
	const isBunVirtualScript = currentScript?.stagetsWith("/$bunfs/root/");
