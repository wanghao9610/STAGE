...
export default function (pi: ExtensionAPI) {
    ...
    const hook = (cwd: string, name: string) => join(cwd, CONFIG_DIR_NAME, "extensions", "stage-hooks", name);
    ...
    pi.on("before_agent_start", async (_event, ctx) => {
        ...
        const parts = [
            await context(ctx.cwd, "stage_model_id.sh", [injectedModel]),
            await context(ctx.cwd, "stage_memory.sh"),
        ].filter((part) => part.length > 0);
        if (parts.length === 0) return;

        return {
            message: {
                customType: "stage-context",
                content: parts.join("\n\n"),
                display: false,
            },
        };
    ...
    pi.on("tool_call", async (event, ctx) => {
        ...
        const { code, text } = await run(hook(ctx.cwd, "stage_commit_guard.sh"), [event.input.command]);
