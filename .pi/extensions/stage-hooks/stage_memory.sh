#!/usr/bin/env bash
# STAGE session hook (Pi) — print the project's memory index so a session starts
...
# message the model sees is .pi/extensions/stage-hooks/index.ts's job, which is also why
