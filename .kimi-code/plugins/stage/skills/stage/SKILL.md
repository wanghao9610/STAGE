...
When `.env` sets `STAGE_LANG=zh`, or it is unset and the conversation is in Chinese, use `.agents/commands/stage.zh-CN.md` for the user-facing wording while preserving the English roster's skill names and routing decisions.
...
For a skill marked `†`, request the confirmation required by the roster, show the exact `/skill:stage-<name> <argument>` invocation, and wait.
