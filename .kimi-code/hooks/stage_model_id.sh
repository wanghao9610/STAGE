#!/usr/bin/env bash
# STAGE UserPromptSubmit hook (Kimi Code) — inject the session's model id into
...
# writing-workflow-conventions §8, so a mid-session switch can leave a stale
# value.
#
...
if [ -n "${sid}" ]; then
  marker="${TMPDIR:-/tmp}/stage_kimi_model_id_${sid}"
