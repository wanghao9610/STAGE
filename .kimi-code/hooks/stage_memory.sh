#!/usr/bin/env bash
# STAGE UserPromptSubmit hook (Kimi Code) — inject the project's memory index
...
# .kimi-code/hooks/install.sh once, which registers the hook for you.
...
shared="$(entries "${root}/.stage/memory")"
machine="$(entries "${root}/.stage/memory/local")"
