#!/usr/bin/env bash
# STAGE PreToolUse hook (Kimi Code) — decline the git commands that break the
...
deny() { # $1 = one-line reason
    printf '{"hookSpecificOutput":{"permissionDecision":"deny","permissionDecisionReason":"%s (declined by .kimi-code/hooks/stage_commit_guard.sh — hand it to the user to run)"}}\n' "$1"
    exit 0
}
