...
调用方式：`/skill:stage-tabs-builder [TABLE]`——`TABLE` 按 ID（`T2`）、文件 slug（`main_results`）或用途短语，对着 `notes/outline.md` 的 Tables 表匹配，§5 的匹配方式套用到 Tables 行上；缺失或有歧义时，列出各行及其状态并提问（§7）。还不在提纲里的表格，在参数里描述它，先给它一行提纲行。一次调用一张表。
