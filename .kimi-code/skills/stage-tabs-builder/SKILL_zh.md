...
1. 汇报：有来源的单元格 vs `\todo{}`（计数与文字）、读过的证据文件及其时间戳、过期发现、动过的台账行、施加的最优标记及覆盖它们的主张。建议下一步：用 `/skill:stage-clms-auditor` 验证这些数字、用 `/skill:stage-sect-drafter` 写表格周围的散文、用 `/skill:stage-evid-curator` 补还缺的东西。
...
- 聊天汇报：有来源/todo 的单元格计数、读到的时间戳、过期发现、台账增减，以及建议的下一个 `/skill:stage-*` 步骤。除这些文件之外什么都不写。
