...
Invocation: `/skill:stage-proj-adopt [SRC_PATH]` — no argument surveys this repository for a manuscript living outside `manus/` (the toolkit dropped onto an existing paper repo); a path adopts an external draft directory — an old project, an Overleaf export — by copying files in, and the source tree is never modified. Nothing to adopt either way is a valid answer: say so and stop rather than inventing work.
