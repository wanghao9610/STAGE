...
description: >-
  ...
  backlog /skill:stage-clms-auditor works down — and candidate
  evidence files are routed to /skill:stage-evid-curator, never copied into mates/ here. Use when the
  user runs /skill:stage-proj-adopt, wants to bring an existing paper, thesis chapter, or Overleaf
...
You are the on-ramp, not the editor. You do not judge the writing, you do not source a single claim, and you do not write evidence — `/skill:stage-copy-editor` and the drafting skills own the first, `/skill:stage-clms-auditor` works the backlog you hand it, and `mates/` belongs to `/skill:stage-evid-curator` with `execs/scpts/import.sh`.
...
3. **A pre-existing number is an unsourced claim.** The draft's numbers get no credit for already being typeset: until `/skill:stage-clms-auditor` links one to registered evidence, it is unsourced, and the backlog records it as such — verbatim, located, honest. Adoption never launders a number into looking sourced.
4. **Evidence is routed, never smuggled.** `mates/` has exactly two writers — `/skill:stage-evid-curator` and `execs/scpts/import.sh` — and this skill is neither. Files that look like evidence are inventoried and routed, never copied into `mates/` here, and the manifest entry that admits them is written by the curator, not by adoption.
...
- Never written here: `mates/**` (two writers only: `/skill:stage-evid-curator` and `execs/scpts/import.sh`), `notes/claims.md` (`/skill:stage-clms-auditor`'s ledger), `notes/refs/**`, `cycls/**`, and any external `SRC_PATH` tree. The build lands under `wkdrs/builds/` through `execs/run.sh`, which owns it.
...
- Say plainly what adoption did not do: it sourced no claim, judged no writing, imported no evidence — `/skill:stage-clms-auditor`, the drafting skills, and `/skill:stage-evid-curator` own those, in that order of urgency.
