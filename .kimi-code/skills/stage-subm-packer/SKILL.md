...
description: >-
  ...
  never edits the manuscript. Use when the user runs /skill:stage-subm-packer, or asks to package,
...
Invocation: `/skill:stage-subm-packer [camera]` — no argument packs a review submission for the active
...
4. **Hard gates.** One is cheaper than the scripts and runs first: a `notes/adopt.md` whose
   ...
   limit → /skill:stage-copy-editor; identity leak → name the file and line.
...
**blocked (n)** with each blocker and the /skill:stage-* skill that clears it.
