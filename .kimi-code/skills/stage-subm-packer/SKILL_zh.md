...
调用方式：`/skill:stage-subm-packer [camera | convert] [kit=<path>]`——不带参数则为当前周期打一个送审包，周期按规约 §5 从 `notes/story.md` 的 `cycle:` 解析；`camera` 为同一个周期打 camera-ready 包，并把承诺关口装上；`convert` 只把论文改排成本周期的 venue 模板并报出该版式下的页数，跳过所有冻结关口。`kit=<path>` 在转换前注册一份官方 venue 模板包（zip 或目录）。无法识别的参数则点名这三种模式并提问。
