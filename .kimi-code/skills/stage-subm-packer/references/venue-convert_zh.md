...
由 venue 自己的宏（第 2 步）填上从 `manus/main.tex` 读出的值构成。要跟着那份导言区里可达的 `\input` 链走：值并不总是字面写在 `main.tex` 里——`/skill:stage-outl-planner` 会把 abstract 移进 `manus/secs/0_abstract.tex`，只在原处留一个 `\input`。
...
- **打包运行，超限**——硬阻断，路由给 `/skill:stage-copy-editor`。
