...
description: >-
  ...
  name the claim IDs they attack — so /skill:stage-resp-writer treats simulated and real reviews
  identically. quick mode runs a single-pass version. Builds first via execs/run.sh; a broken
  build is finding #1. Never edits the manuscript or the claim ledger. Use when the user runs
  /skill:stage-peer-reviewer, or asks for a mock review, a review panel, a pre-submission attack on
