...
finding for `/skill:stage-figs-designer` — never a re-plot here (SKILL.md Principle 4).
...
2. **`% src:` sits on the line above the number it sources**, one comment per number — the same
   discipline `/skill:stage-tabs-builder` applies to table rows, so `/skill:stage-clms-auditor` can walk the
...
`/skill:stage-subm-packer`'s Principle 7 boundary, and it holds for the same reason — a class edited to
