...
description: >-
  ...
  the latest build and lint signal, and exactly one next action with its exact /skill:stage-* command.
  Reports in chat only and points every action at the sibling skill that owns it. Use when the user runs
  /skill:stage-flow-status, when a run names it as the next action, or asks where the paper stands, what
  to work on next, whether evidence or the build is fresh, or how far the current cycle has gotten. Never
  writes.
---
...
Invocation: `/skill:stage-flow-status [SECTION] [DESCRIPTION]` — no argument reports the whole
flow; a section argument, resolved per conventions §5 by number, file slug, or title against
...
1. **Strictly read-only.** Never create, edit, or delete any file — not the outline, not the
   ...
   /skill:stage-resp-writer, /skill:stage-subm-packer.
...
5. **One recommendation, chosen by the priority order.** End with a single next action and its
   exact /skill:stage-* command, picked by Workflow step 7 — not a menu. Everything else outstanding
...
4. **Claim coverage.** Ledger counts by status: proposed / drafted / verified / unsourced /
   weakened / dropped. `unsourced > 0` is always a gap line naming /skill:stage-clms-auditor.
5. **Evidence, refs, and style.** MANIFEST entry count and newest `imported:`; `import.sh --diff`
   ...
   reading note is /skill:stage-refs-curator's. Then one line for the style profile (§8.11): present
...
7. **Next action.** First match wins: (1) no `notes/adopt.md` → /skill:stage-proj-adopt; (2) story
   missing or unfinalized → /skill:stage-stry-coach; (3) outline missing or unfinalized →
   /skill:stage-outl-planner; (4) evidence drifted → /skill:stage-evid-curator; (5) open promises → the
   skill the first open box's change needs (/skill:stage-sect-drafter, /skill:stage-tabs-builder,
   /skill:stage-figs-designer); (6) an outline row still planned / skeleton / sketch → its owner among
   those three, with the row named; (7) claims at `unsourced`, or `drafted` never verified →
   /skill:stage-clms-auditor; (8) all rows drafted but the newest `CITES_*` / `POLISH_*` report date
   trails the outline's `updated:` → /skill:stage-cite-auditor, then /skill:stage-copy-editor; (9) no
   simulated review this cycle → /skill:stage-peer-reviewer; (10) all green → /skill:stage-subm-packer.
   ...
   **A red gate outranks the list.** When step 6 found `lint.sh` failing hard — the build broken, a `\todo{` that would ship, a page count over the limit, an identity leak under `ANON=true` — that is the next action whichever numbered rule matched, routed to the owner lint itself names: a marker to `/skill:stage-sect-drafter` or `/skill:stage-tabs-builder`, an over-limit paper to `/skill:stage-copy-editor`, an undefined citation to `/skill:stage-cite-auditor` or `/skill:stage-refs-curator`. Nothing downstream of a red gate is worth recommending — `/skill:stage-subm-packer` refuses it, and a simulated review of a manuscript that does not build reviews the wrong artifact. The list resumes once the gate is green.
...
provenance → gap lines (omitted when none fire) → the one next action with its exact /skill:stage-* command and
