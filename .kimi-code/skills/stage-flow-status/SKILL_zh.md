...
description: >-
  ...
  /skill:stage-* 命令。只在聊天里汇报，并把每个动作指向拥有它的那个兄弟 skill。只要用户运行
  /skill:stage-flow-status，或询问论文进展到哪、接下来该做什么、证据或构建是否新鲜、当前周期走到了哪一步，
  都应使用本 skill。绝不写任何文件。
---
...
7. **下一步动作。** 首个命中者胜出：(1) 没有 `notes/adopt.md` → /skill:stage-proj-adopt；(2) story 缺失或未定稿 → /skill:stage-stry-coach；(3) 提纲缺失或未定稿 → /skill:stage-outl-planner；(4) 证据漂移 → /skill:stage-evid-curator；(5) 有未清承诺 → 第一个未勾选框所需改动对应的 skill（/skill:stage-sect-drafter、/skill:stage-tabs-builder、/skill:stage-figs-designer）；(6) 提纲里仍是 planned / skeleton / sketch 的行 → 上述三者中它的归属者，并点名是哪一行；(7) 主张处于 `unsourced`，或 `drafted` 却从未验证 → /skill:stage-clms-auditor；(8) 所有行都已 drafted，但最新的 `CITES_*` / `POLISH_*` 报告日期落后于提纲的 `updated:` → 先 /skill:stage-cite-auditor，再 /skill:stage-copy-editor；(9) 本周期还没有模拟评审 → /skill:stage-peer-reviewer；(10) 全绿 → /skill:stage-subm-packer。给出一行理由和确切命令。
