...
5. **证据与参考文献。** MANIFEST 条目数与最新的 `imported:`；`import.sh --diff` 的结论（clean / drifted / unknown）；bib key 对着 refs 索引行——被引用却没有阅读笔记的工作是 /skill:stage-refs-curator 的活。
...
汇报顺序：周期状态 → 提纲面板 → 主张覆盖 → 证据与参考文献 → 构建与 lint → 溯源 → 缺口行（一条都没触发就省掉）→ 唯一的下一步动作，连同它确切的 /skill:stage-* 命令与理由。用紧凑的表格与计数，绝不逐行写散文；字段缺失处写 "unknown"；整条回复控制在约 500 词以内。
