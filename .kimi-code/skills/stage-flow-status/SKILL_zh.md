...
description: >-
  整条写作流程的只读地图：从 notes/outline.md 读出每节、每图、每表的状态，按记录表状态统计主张覆盖，对着上游时间戳检查证据新鲜度，参考文献计数，周期状态（venue 是否确认、评审是否到齐、回复是否起草、承诺是否未清、是否已冻结），最近一次构建与 lint 信号，以及恰好一个下一步动作连同它确切的 /skill:stage-* 命令。只在聊天里汇报，并把每个动作指向拥有它的那个兄弟 skill。只要用户运行 /skill:stage-flow-status、一次运行点名它是下一步动作，或询问论文进展到哪、接下来该做什么、证据或构建是否新鲜、当前周期走到了哪一步，都应使用本 skill。绝不写任何文件。
---
...
调用方式：`/skill:stage-flow-status [SECTION] [DESCRIPTION]`——不带参数汇报整条流程；带章节参数时，按规约 §5 以编号、文件 slug 或标题对着 `notes/outline.md` 解析，把提纲面板与主张明细收敛到该节。`involve=<level>` 记号在解析 SECTION 之前就被剥离（§7），除此之外在这里不改变任何行为。章节参数有歧义是本 skill 唯一可以问的问题（§5）；除此之外它什么都不问。`SECTION` 之后剩下的文字是一句描述（规约 §7.13）：用你自己的话说明这次运行是为了什么。解析不到任何章节的散文就是纯描述，不是缺目标——照旧汇报整条流程，并在回复第一行说明这一点。描述可以引导报告把哪里读得最细——哪个未知值得再看一眼、哪一行值得原样引出来而不是只数个数——但它不会漏掉报告该给的任何一项，也绝不挪动那个下一步动作，那由优先级顺序一路定到并列时的决胜规则。
...
5. **只给一条建议，由优先级顺序选出。** 以单一的下一步动作及其确切的 /skill:stage-* 命令收尾，由 Workflow 第 7 步选出——不是一份菜单。其余尚未了结的事情留在缺口行里。没有任何一条够格时，点名那个卡点。
...
4. **主张覆盖。** 记录表按状态计数：proposed / drafted / verified / unsourced / weakened / dropped。`unsourced > 0` 永远是一条点名 /skill:stage-clms-auditor 的缺口行。
5. **证据、参考文献与文风。** MANIFEST 条目数与最新的 `imported:`；`import.sh --diff` 的结论（clean / drifted / unknown）；bib key 对着 refs 索引行——被引用却没有阅读笔记的工作是 /skill:stage-refs-curator 的活。然后给文风档案（§8.11）一行：存在就报它的 `source:` 与 `updated:`，不存在就说不存在——不存在是一个仓库的默认状态，不是缺口，也永远不会成为下一步动作。
...
7. **下一步动作。** 首个命中者胜出：(1) 没有 `notes/adopt.md` → /skill:stage-proj-adopt；(2) story 缺失或未定稿 → /skill:stage-stry-coach；(3) 提纲缺失或未定稿 → /skill:stage-outl-planner；(4) 证据漂移 → /skill:stage-evid-curator；(5) 有未清承诺 → 第一个未勾选框所需改动对应的 skill（/skill:stage-sect-drafter、/skill:stage-tabs-builder、/skill:stage-figs-designer）；(6) 提纲里仍是 planned / skeleton / sketch 的行 → 上述三者中它的归属者，并点名是哪一行；(7) 主张处于 `unsourced`，或 `drafted` 却从未验证 → /skill:stage-clms-auditor；(8) 所有行都已 drafted，但最新的 `CITES_*` / `POLISH_*` 报告日期落后于提纲的 `updated:` → 先 /skill:stage-cite-auditor，再 /skill:stage-copy-editor；(9) 本周期还没有模拟评审 → /skill:stage-peer-reviewer；(10) 全绿 → /skill:stage-subm-packer。给出一行理由和确切命令。当这条命令落在那十个 agent 可以自己启动的 skill 上、且目标已经定死时，它会在这份报告写完之后被直接拾起来跑，而不是留给作者去敲——本 skill 自己不启动任何东西（规约 §11.4）。

   **红闸门压过这张表。** 当第 6 步发现 `lint.sh` 硬失败时——构建坏了、一个会被印进 PDF 的 `\todo{`、页数超限、`ANON=true` 下的身份泄漏——那就是下一步动作，无论上面哪条编号命中，并路由给 lint 自己点名的归属者：标记归 `/skill:stage-sect-drafter` 或 `/skill:stage-tabs-builder`，超页归 `/skill:stage-copy-editor`，未定义引用归 `/skill:stage-cite-auditor` 或 `/skill:stage-refs-curator`。红闸门下游的任何事都不值得推荐——`/skill:stage-subm-packer` 会拒绝它，而对一份编译不出来的手稿做模拟评审，评的是错的产物。闸门转绿后这张表继续生效。
...
汇报顺序：周期状态 → 提纲面板 → 主张覆盖 → 证据、参考文献与文风 → 构建与 lint → 溯源 → 缺口行（一条都没触发就省掉）→ 唯一的下一步动作，连同它确切的 /skill:stage-* 命令与理由。用紧凑的表格与计数，绝不逐行写散文；字段缺失处写 "unknown"；整条回复控制在约 500 词以内。
