...
description: >-
  ...
  every fix to /skill:stage-refs-curator or /skill:stage-sect-drafter. Use when the user runs
  /skill:stage-cite-auditor, or asks whether the citations and related-work claims hold up.
---
...
7. **File failures.** Append one `- [ ]` per undefined key, unsupported or unverifiable
   ...
   the paper into one; wrong sentence → `/skill:stage-sect-drafter`; bib repair → `/skill:stage-refs-curator`.
