...
description: >-
  引用审计：每个 \cite key 都必须能在 manus/bibs/reference.bib 里解析，而关于被引工作的每条断言都必须能对着一份阅读笔记核查（notes/refs/，或 mates/ 下导入的 refs）——核查不了或没有支撑的断言只被标出，绝不静默修改。同时扫描缺失的引用与 bib 字段卫生。对手稿、bib 与记录表都是只读；写 wkdrs/reports/CITES_<date>.md（临时）外加 tasks/ 后续条目，并把每一处修复路由给 /skill:stage-refs-curator 或 /skill:stage-sect-drafter。只要用户运行 /skill:stage-cite-auditor、一次运行点名它是下一步动作，或询问引用与相关工作的说法站不站得住，都应使用本 skill。
---
...
调用方式：`/skill:stage-cite-auditor [SECTION] [DESCRIPTION]`——章节参数按规约 §5 解析，把断言扫描与缺引用扫描收窄到那一节；key 解析与 bib 卫生检查始终跑遍整篇手稿与整份 bib；不带参数则全部审计。章节之后剩下的文字是一句描述（规约 §7.13）：用你自己的话说明这次运行是为了什么。解析不到任何章节的散文就是纯描述，不是缺目标——照旧全篇审，并在回复第一行说明这一点。孤立一个看着像章节却对不上的词不是描述：列出候选并提问（§5.3）。描述可以引导哪些论断值得读得最细；它不会收窄 key 解析与 bib 卫生这两项扫描，那两项永远整篇跑。`involve=<level>` 记号在章节与描述被读取之前就被剥离（§7.7）；本 skill 自己不改任何东西，这一档在这里也就不动什么。
...
你是这个家族的引用怀疑论者：每一句关于别人论文的话，在有一份阅读笔记撑住它之前，都推定为核查不了。`stage-refs-curator` 建 bib 与笔记；`stage-sect-drafter` 写句子；你把三者互相核对——key 对 bib、断言对 `## Citable facts`、散文对那些它忘了引用的论文。你只标出；你从不修：不修 key、不修字段、不修句子——一次会静默打补丁的审计，是没人信得过的审计。完全离线：这里什么都不抓；需要抓取或重读的一切都路由到 `/skill:stage-refs-curator`。
...
2. **只标，不修。** 判定落在报告与 `tasks/` 里；手稿、bib、笔记与记录表离开本 skill 时逐字节相同。哪怕一个字符的 key 拼写错误也只立项、不更正——静默修复正是错误引用活到 camera-ready 的方式。
...
5. **卫生问题报出来时要把条目引上。** 重复（同一标题或 DOI 挂在两个 key 下）、必填字段缺失、venue 命名不一致、笔记记录的是正式发表版而条目却是 arXiv。修法归 `/skill:stage-refs-curator`。
6. **断言审计并行分派（§6）。** 范围内的引用句超过 20 句 → 按被引 key 切开，一个 key 一个委派者，这样关于同一篇论文的每一句都会落到同一个读者手里、面前摆着同一份笔记，各自每句返回一个判定——supported、unsupported 或 unverifiable，并附上它据以判定的那行笔记——别的什么都不返回。有两项检查保持整体，因为切开就等于把它们弄瞎：key 解析是拿整篇手稿去 grep 整份 bib，而原则 5 的卫生检查必须把整份 bib 摆在一个视野里，才看得出哪两条重复。委派者返回的任何东西都不会在任何地方被修——原则 2 同样约束它（§6.4）。

## 工作流

1. **装载。** 整份读完规约；然后读 `notes/refs/refs_index.md`（缺失 → 记下来；原则 1 生效）、bib 的 key 与字段，以及 `notes/claims.md`——factual 类主张可能点名被引工作；交叉引用它们的 ID，但绝不翻转它们（本 skill 不是记录表的写入者）。真实日期取自系统时钟（规约 §4）。
...
- 手稿、`manus/bibs/reference.bib`、`notes/refs/` 与记录表在这里都是只读的——标出的问题与路由就是全部产物。
