...
description: >-
  ...
  in notes/style.md and edits no prose. Use when the user runs /skill:stage-copy-editor, when a run names it as the
  next action, or asks to polish, tighten, proofread, or de-jargon the paper's prose, or to set,
...
Invocation: `/skill:stage-copy-editor [SECTION | style]` — a section argument resolves by number, file
...
cutting substance, reordering an argument, rewriting a claim — reaches `/skill:stage-sect-drafter` as a
...
1. **Numbers are not prose (conventions §9a).** Never change, round, reformat, delete, or move
   ...
   stays. A suspicious number is a finding for `/skill:stage-clms-auditor`, never a fix here.
2. **The canon rules the words.** `notes/notation.md` is law: every term in its Never column is
   ...
   flagged for `/skill:stage-sect-drafter` to register — this skill enforces the canon, never extends it.
...
4. **Budgets come from the outline.** Trim toward each section's Budget column in
   ...
   substance becomes a routed finding naming what must go — the cut belongs to `/skill:stage-sect-drafter`.
...
2. **Resolve scope (conventions §5).** The literal `style` → the profile branch below, and
   ...
   **The profile branch (`style`).** Follow `references/style-profile.md`, which holds the dial
   ...
   closing line is `/skill:stage-copy-editor <section>` — the run that puts the dials to work.
