...
调用方式：`/skill:stage-copy-editor [SECTION | style]`——章节参数按编号、文件 slug 或标题对着 `notes/outline.md` 解析（规约 §5；有歧义 → 提问）；不带参数则按提纲顺序，打磨提纲里状态为 `drafted` 或更靠后的每一节。字面量 `style` 改走档案分支：它写 `notes/style.md`，一个字的散文都不碰；`style preset:<name>` 从该预设起手，`style sample=<path>`（可重复）量那些文件，未识别的 token 一律问、绝不猜。
