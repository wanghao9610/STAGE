...
6. **翻转是挣来的——两个方向都是。** 一条主张翻成 `verified`——从 `drafted`，或从 `unsourced`（前提是它的陈述已经甩掉了那个状态所要求的 `\todo`）——只在它名下每个数字都对上了新鲜的证据、且它所有证据链接都能解析时发生：上一次审计点名的债，由找到踪迹的这次审计清账，绝不由谁记得它还过了来清。它名下只要有一个裸的无来源数字 → `unsourced`。此前是 `verified` 的主张今天没通过就失去这个状态——退回 `drafted`，并附一条说明为什么的任务。一次 mismatch 永远不会把任何东西往上翻。`weakened` 是本审计唯一不抬的状态：它标记的是对评审人做出的让步，只有按 `tasks/<cycle>_promises.md` 里已勾选的承诺重述该主张的那次修订——`stage-sect-drafter` 或 `stage-tabs-builder`——才把它送回 `drafted`，此后核验走寻常路径。
...
3. **过期关口。** 跑 `execs/scpts/import.sh --diff`（Bash）并记录结果：干净，或者漂移 / 上游新增 / 上游缺失的清单。漂移的路径污染 match（原则 5）。没有配置 STAR 来源 → 记下来并继续；MANIFEST 指纹仍然是参照。
...
5. **分桶。** 被 `\todo{}` 包住的数字 → declared-unsourced：这是 §9a 的合法状态——计数并上记录表，不开违规任务（`lint.sh` 数它们；本次审计解释它们）。归属被引工作的数字 → 交给 `stage-cite-auditor` 的移交清单（原则 4）。其余一切 → 去追踪迹。
...
7. **翻转记录表。** 对 `notes/claims.md` 里范围内的每条主张施加原则 6；踪迹找到了记录表 Evidence 列缺少的锚点时，把它记上；把 frontmatter 的 `updated:` 设为真实日期。记录表翻转是本次审计的持久成果（规约 §10：`wkdrs/` 永不提交）。
   ...
   接入的待办行在这里进入记录表，也只经由本 skill 进入（规约 §9a：每个已知但未指纹化的取值都要有一条 `unsourced` 记录）。`notes/adopt.md` 待办里尚未进表的每一行，本次运行都给它开一行——下一个空闲 ID、按手稿原样陈述的主张、它所在的章节或表格写进 `Stated in`、本次追踪解析出锚点则写进 Evidence 否则记 `—`、状态按本次判定给：名下每个数字都对上新鲜证据 → `verified`，否则 → `unsourced`——而每条 `unsourced` 都是第 8 步的一条任务，路由给将为这个数字补来源、或把它改写成其状态所要求的 `\todo` 的那个 skill。
