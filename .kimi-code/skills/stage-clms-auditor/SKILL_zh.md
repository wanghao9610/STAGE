...
7. **追溯并行分派（§6）。** 范围里的 `.tex` 文件超过一个 → 范围内每个 `manus/secs/` 与 `manus/tabs/` 文件一个委派者，各自对自己那些数字按原则 3 的顺序走，把被引的 `mates/` 文件在它的锚点处打开，每个数字返回一行——tex 里的值、锚点处的值、判定——别的什么都不返回。`notes/claims.md` 只有一个写入者，而且是主 agent：Step 7 要把所有委派者的行同时读一遍，一条主张会在不止一个文件里被陈述，而两个委派者一起改的那行台账，总有一个的修改会丢（§6.2）。过期关卡先跑，并且在这里跑——它决定本次运行产出的判定究竟能不能把任何东西往上翻。
...
5. **分桶。** 被 `\todo{}` 包住的数字 → declared-unsourced：这是 §9a 的合法状态——计数并上台账，不开违规任务（`lint.sh` 数它们；本次审计解释它们）。归属被引工作的数字 → 交给 `/skill:stage-cite-auditor` 的移交清单（原则 4）。其余一切 → 去追踪迹。
