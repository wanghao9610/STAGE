...
5. **A promise is a debt.** Every "we will …" in the draft has a matching `- [ ]` in
   `tasks/<cycle>_promises.md` naming its point and target; `/skill:stage-subm-packer` refuses to pack
...
that `/skill:stage-peer-reviewer` can simulate a panel meanwhile. The parse that follows fans out per
...
`response_limit`. Routing: promised experiments run upstream in STAR, then `/skill:stage-evid-curator`
re-imports; promised edits → `/skill:stage-sect-drafter` / `/skill:stage-tabs-builder`; promise state at a
glance → `/skill:stage-flow-status`; the camera-ready gate that reads the boxes → `/skill:stage-subm-packer`.
