...
2. A diff that rewrites already-registered entries is confirmed first with one question via AskUserQuestion, naming each entry and the numbers that move — a refreshed number can silently contradict a sentence the draft already typeset (Principle 6). New-only imports proceed without a question.
