...
<!-- 由 /skill:stage-refs-curator 写出。本文件是 manus/bibs/reference.bib 的审计线索：每条条目的出处
...
<!-- 驱动本次运行的是什么：论文的 story 与主张台账、mates/ 下导入的 refs 树，或用户给的
     ...
     只有这份记录能把它和"根本没查"分开。discover 那一轮还要点名是什么让扫掠结束的——某条查询
     回来全是已见过的，或者画像里的查询种类都出过了——以及引文图是在哪些候选上跑的，因为扩展
     只许碰扫掠产出的东西。这两样都从这份记录里读出来，而不是在旁边另写一句。 -->

## 2. 有笔记的论文

<!-- notes/refs/ 下每份阅读笔记一行。"为什么在这里被引"写的是本篇稿件的理由，不是那篇论文的
     摘要。影响力分抄自第 5 节的总分，放这里是为了好读。阅读深度一栏：本地写的笔记写 `—`，
     因为这里的阅读是一条底线而不是一把刻度；播种来的笔记逐字抄上游笔记的 depth，其中
     `abstract-and-intro` 标记的是一篇仍然值得正式读一遍的论文。 -->

| Citekey | 笔记 | 会议/期刊 | 为什么在这里被引 | 阅读深度 | 影响力分 | Model |
| --- | --- | --- | --- | --- | --- | --- |
...
<!-- 每条一行，算式与分档见 source-policy_zh.md"影响力分"一节：子指标带各自的抓取日期，然后是
     ...
     度，/skill:stage-refs-curator score 重建整表。 -->
...
<!-- 值得再跑一轮补的缺口（某个类别偏薄、被引却没有笔记的工作、方案没碰的历史 citekey），
     ...
     下一次运行提议点新的、而不是同一份清单。
     转交：拿这些笔记核验稿件里的断言 → /skill:stage-cite-auditor；按聚类起草 Related Work →
     /skill:stage-sect-drafter；以后单加一篇 → /skill:stage-refs-curator <arxiv-id>；去找没人点过名的
     工作 → /skill:stage-refs-curator discover；重查整个 bib →
     /skill:stage-refs-curator verify；引用与星标漂了 → /skill:stage-refs-curator score。 -->
