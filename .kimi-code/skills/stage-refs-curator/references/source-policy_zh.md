...
阅读那一步扇出时，一个委派者做什么、返回什么（`SKILL_zh.md` 原则 9）。一篇论文一个。委派者自己去抓那篇论文自己的页面——规约 §6.9——两次自己的请求之间按任务书写明的秒数等待，每份原始内容都先缓存到任务书点名的前缀下再读。别的 URL 一概不开：不抓书目记录，不打检索端点，不调 GitHub API，这些都留在主 agent 手里。
...
**§9b 约束的是写下那条要点的人，在这里就是你。** 一条 `## Citable facts` 就是一句还没写出来的手稿断言，而 `/skill:stage-cite-auditor` 日后要只凭你这份笔记、不重新打开论文，就对一句手稿下判定。所以每条要点都是一个自足的事实——数字要带着它的数据集、指标与设置一起走——点名它出自哪一节、哪张表、哪个公式，并带上从缓存页面里逐字符抄下来的至多 25 个词。你没法钉到本次运行读过的东西上的事实，不进这个文件；而把转述当成引文写下，正是这条规则要挡的那种失败。
