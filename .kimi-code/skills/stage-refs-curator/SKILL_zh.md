...
2. **笔记存在的意义就是被拿来核（§9b）。** `## Citable facts` 是与 `/skill:stage-cite-auditor` 的合同：每条事实一个自足的要点——一个数字要带着它的数据集、指标与设置一起走；一条方法或范围的主张要点明论文在哪里这么说（章节、表格，或一句短引文）。判据是：审计器必须仅凭笔记就能对一句手稿下结论，不必重开论文。没有钉在本次运行真读过的东西上——或它所引用的上游 STAR 笔记所陈述的东西上——的事实，不得进入。
...
9. **阅读可以扇出，读的人自己抓页面（§6）；笔记仍在家里写。** 一次运行里排队要读的论文有三篇或更多，就按一篇一个派只读委派者，同时最多三个（§6.2）；不足三篇由主 agent 自己读——那时派发的成本超过它省下的那次阅读（§6.1）。委派者抓的正好是它被派去读的那篇论文的页面——§6.9 那个唯一的口子——两次自己的请求之间按任务书写明的秒数等待（host 的间隔乘以在跑的委派者数），并把每份原始内容缓存到运行目录下它自己的前缀里（§6.4）。别的一概不动：每个 bib 字段所誊写的书目记录、发现检索，以及那一次 GitHub 指标调用，都留在主 agent 手里，一次一个请求。交给委派者的是那份任务书与 `references/source-policy_zh.md` 里的收集者合同；它交回来的是那份合同的字段，此外什么都没有。它不写任何仓库文件：`## Relation to ours` 正是这份笔记存在的理由，它需要故事与台账，所以笔记、索引行与 bib 条目全是主 agent 的。它返回的每条事实都带一句逐字引文，写笔记之前先到缓存页面里核实（§6.5），因为一条 `## Citable facts` 就是一句还没写出来的手稿断言（§9b）——缓存里搜不到的引文丢掉它那条事实，而一份撑不起阅读底线的返回，让那篇论文只留下 bib 条目、不写笔记，与正文抓不到时完全一样。
...
5. **提问（§7），而且这一问是强制的（§7.7）。** 一次直接提问，带上通过了 Step 4 的那些候选，最相关的在前，每条给标题、venue、年份、那句为什么、临时分数；推荐其中一个子集并说明为什么是这几条。那句"为什么"就是筛子，旁边不需要再放一个上限：一条候选如果说不出一句话的理由，它本来就不是候选。什么能进这个参考文献底盘，在任何 involve 档位下都是作者的决定——一份按代理自己的口味攒起来的底盘，评审时没人替它辩护得了。用户可以往这一组里加自己的论文，加进来的按任何其他输入一样解析。
...
**把阅读扇出去（原则 9）。** 一次运行里有三篇或更多——一份 `add` 清单，或 `discover` 那一轮用户挑中的一批——就按一篇论文一个只读委派者拆开，同时最多三个——用`coder` 类型的 `Agent` 子代理，在同一条消息里一次派出去，它们才是并行的。调用本 skill 本身就是在要它们；宿主根本不提供派发工具时，退到规约 §6.1 的本地路径，并在摘要里说明退了，以及随之丢掉的是哪一道检查。第 1、2 步仍在家里逐篇跑完：条目是照着书目记录誊的，而委派者不抓记录。出门的是第 3 步，每份任务书带着它那篇论文的候选页面（顺序照第 3 步点的名）、它对某台 host 两次自己的请求之间等多少秒（`references/source-policy_zh.md`，限速一节）、它自己在 `wkdrs/refs_<date>/raw/` 下的缓存前缀、逐字的收集者合同，以及范围那一句"只读这一篇；只抓它自己的页面；只返回合同里的字段，别的什么都不返回"。involve 为 `high` 时，派发之前先把分工说出来（§6.8）。第 4 步随后逐篇在家里跑：先打开那个委派者的缓存前缀，再谈它的返回——`pages_fetched` 点的名在磁盘上一个都不存在，就等于什么都没证明，那篇论文回到这里重读；然后每条返回的引文都要到缓存页面里搜一遍，搜不到的那条事实丢掉并点名，`floor_evidence` 写着 `not reached` 的那一份让 bib 条目留着、不写笔记——那就是第 3 步里正文抓不到那条路的扇出形态——而 `## Relation to ours` 是拿委派者的原始素材、对着故事与台账写出来的，绝不照抄它的行文。
...
5. 报出簇的地图，并把偏薄的簇作为"接下来该读"的清单给出（一条 `/skill:stage-refs-curator add …`）；据它起草相关工作是 `/skill:stage-sect-drafter` 的事。
...
2. 在聊天里给摘要：新增 / 播种 / 失败 / 待人工核查的条目数、写出的笔记（`ABBREV` → 文件）、`discover` 那一轮提议 / 收进 / 搁置了多少条候选并点名稀薄与落空的查询、卫生修复、分数表最上面那几条（带 `*` 或 `new` 的要解释）、方案没碰的历史 citekey、簇的地图或"接下来该读"清单、阅读怎么扇出的以及扇出时缓存核对丢掉了什么，以及路由——核验手稿断言 → `/skill:stage-cite-auditor`；起草相关工作 → `/skill:stage-sect-drafter`；导入上游 refs 树 → `/skill:stage-evid-curator`。
