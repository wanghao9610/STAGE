...
9. **阅读可以扇出，笔记仍在家里写（§6）。** 一次运行里排队要读的论文有三篇或更多，就按一篇一个派只读委派者，同时最多三个（§6.2）；不足三篇由主 agent 自己读——那时派发的成本超过它省下的那次阅读（§6.1）。委派者一律不抓取（§6.4、§6.9）——每个论文页面都已经由主 agent 按礼貌速率抓好并缓存，所以交给委派者的是磁盘上的一个文件、`references/source-policy_zh.md` 里的收集者合同，以及它的范围那一句；它交回来的是那份合同的字段，此外什么都没有。它不写任何文件：`## Relation to ours` 正是这份笔记存在的理由，它需要故事与台账，所以笔记、索引行与 bib 条目全是主 agent 的。它返回的每条事实都带一句逐字引文，写笔记之前先到缓存页面里核实（§6.5），因为一条 `## Citable facts` 就是一句还没写出来的手稿断言（§9b）——缓存里搜不到的引文丢掉它那条事实，而一份撑不起阅读底线的返回，让那篇论文只留下 bib 条目、不写笔记，与正文抓不到时完全一样。
...
**把阅读扇出去（原则 9）。** 一次运行里有三篇或更多——一份 `add` 清单，或 `discover` 那一轮用户挑中的一批——就按一篇论文一个只读委派者拆开，同时最多三个。顺序是要紧的：第 1、2 步先为每一篇跑完，记录抓好、条目誊好、论文页面缓存好，然后才有委派者开工；每个委派者拿到的是它那份缓存文件、逐字的收集者合同，以及范围那一句"只读这一篇；只返回合同里的字段，别的什么都不返回"。任何一份任务书都不带 URL、不带抓取预算，也不带可写的缓存前缀——已经没有东西要它抓，也没有东西要它写。页面缓存不下来的论文根本不派出去，它走第 3 步里正文抓不到的那条路。involve 为 `high` 时，派发之前先把分工说出来（§6.8）。第 4 步随后逐篇在家里跑：每条返回的引文都要到那篇论文的缓存页面里搜一遍，搜不到的那条事实丢掉并点名，`floor_evidence` 写着 `not reached` 的那一份让 bib 条目留着、不写笔记，而 `## Relation to ours` 是拿委派者的原始素材、对着故事与台账写出来的——绝不照抄它的行文。
