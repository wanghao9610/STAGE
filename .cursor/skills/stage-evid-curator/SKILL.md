...
2. A diff that rewrites already-registered entries is confirmed first with one question via AskQuestion, naming each entry and the numbers that move — a refreshed number can silently contradict a sentence the draft already typeset (Principle 6). A diff with only `new upstream` lines proceeds without a question; the script imports a slug whole, so it still re-stamps that slug's existing entries with the new `source-commit:` and `imported:`, their bytes and `sha256:` unchanged. A declined confirmation imports nothing, since the script cannot import part of a slug: the reply names the new files still waiting and the exact command that imports them together with the refresh.
...
3. One question via AskQuestion for what no probe can know: `source` — where this came from (path, URL, or person) — and `covers`, asked on every `register` (conventions §7.7): what the user's request already said is quoted as the proposed answer, and the entry waits for the user to confirm it.
...
- Every question — an unrecognized token or a missing `--source` (Step 0), the import-rewrite confirmation (Step 1), a name collision, a changed checksum, and the provenance question (Step 2), an obsolete file — goes via AskQuestion, one question per call. If it is unavailable (headless / scripted), fall back to plain text and require an explicit answer before the write.
