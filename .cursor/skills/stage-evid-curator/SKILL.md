...
2. A diff that rewrites already-registered entries is confirmed first via one AskQuestion, naming each entry and the numbers that move — a refreshed number can silently contradict a sentence the draft already typeset (Principle 6). New-only imports proceed without a question.
...
3. One AskQuestion for what no probe can know: `source` — where this came from (path, URL, or person) — and `covers`, skipped when the user's request already said both.
