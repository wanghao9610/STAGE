...
description: >-
  ...
  manuscript or mates/ — every fix routes to the skill that owns the file. Use when the user runs
  /stage-clms-auditor, or asks whether the paper's numbers are backed by evidence, or before any
  submission freeze.
---
...
7. **Single session, no `Task` subagents (conventions §6).** The audit's value is one context that has
