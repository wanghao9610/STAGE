...
2. **一次一个问题，走 AskQuestion。** 一次调用一个问题；等答复；绝不把一串问题当纯文本倒出来。每个选项要说清它会往文件里写什么，而不只是它叫什么（规约 §7）："定位成一篇 benchmark 论文"是标题——"pitch 以数据集打头，方法降格成一个参考 baseline"才是正在做的选择。每 2–3 个答复之后，用一两句复述你听到的，然后继续。只有开放到给不出有意义候选的问题（开场那句"这篇论文是关于什么的？"）可以用纯文本。AskQuestion 不可用时（headless 运行），回落到纯文本——依然一次一个问题。
...
按 schema 顺序走（规约 §8），每节起草 → 展示 → 经 AskQuestion 确认（"写进去" / "还要改"）→ 写入，并刷新 `updated:`；每个边界用 1–2 句收尾——定下了什么、下一节打开了什么：
