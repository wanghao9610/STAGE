...
**通用规约。** 动手之前先读 `docs/mds/stage-workflow/writing-workflow-conventions.md`（中文对照：`writing-workflow-conventions.zh-CN.md`）——整份读，每次运行开始时；不做分节选读。它通过自己的 `Read` 调用到达，绝不被 `cat` 进一条 Shell 命令。它是所有 STAGE skill 共享的基线；对本 skill 约束最紧的是 §9（编造边界——本 skill 就是它在表格层面的执行者）、§8（产物登记表及其过期规则）、§5（解析）。本文件只写本 skill 特有的部分，更严处以本文件为准。
...
1. 把 `TABLE` 依次按 ID、文件 slug、用途短语对着 Tables 行匹配；缺失或有歧义 → 列出各行及状态并经 AskQuestion 提问（不可用时用纯文本）。
