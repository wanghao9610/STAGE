...
3. **Search.** 5–8 queries from the profile — task terms, mechanism terms, the synonyms the field actually uses, benchmark names, the "X for Y" shape papers title themselves with — over the discovery endpoints in `references/source-policy.md`, at the polite rates that file fixes. A thin result set is grown through the citation graph — at most 3 seed papers, one hop, never two — never through more wordings of the same query. Log every query with its hit count, the zero-hit ones included.
...
5. **Ask (§7), and this one is mandatory (§7.7).** One direct question carrying up to ~15 candidates, most relevant first, each with title, venue, year, the why clause, and the provisional score; recommend a subset and say why those. What enters the reference base is the author's call at every involve level — a base assembled by an agent's own taste is one nobody can defend at review time. The user may add papers of their own to the set, which then resolve like any other input.
