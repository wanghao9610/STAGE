...
3. **检索。** 由画像生成 5–8 条查询——任务词、机制词、这个领域实际在用的同义说法、benchmark 名字，以及论文给自己取标题时那种"X for Y"的形状——打到 `references/source-policy_zh.md` 列出的发现端点，按那份文件定死的礼貌速率进行。结果太薄要靠引文图去长——最多 3 篇种子、只走一跳，绝不两跳——不是靠把同一条查询换几种说法。每条查询连同命中数一并记录，命中为零的也记。
