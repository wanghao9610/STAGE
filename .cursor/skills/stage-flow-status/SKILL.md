...
description: >-
  ...
  it. Use when the user runs /stage-flow-status, or asks where the paper stands, what to work on
...
1. **Strictly read-only.** Never create, edit, or delete any file — not the outline, not the
   ...
   no AskQuestion, no plan mode, no `Task` subagents. To act on what you show, point at the owner:
