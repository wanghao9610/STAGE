#!/usr/bin/env bash
# STAGE sessionStart hook (Cursor) — inject the runtime-reported model id into
...
# When neither is present the honest value per writing-workflow-conventions §8 is
...
if [ -n "${model:-}" ]; then
  ctx="STAGE provenance: this session's runtime-reported model id is ${model}. When a STAGE skill records a model_id or a model_trail entry (writing-workflow-conventions section 8), copy this exact string verbatim; do not write 'unrecorded'."
else
