#!/usr/bin/env bash
# STAGE beforeShellExecution hook (Cursor) — decline the git commands that break
...
deny() { # $1 = one-line reason
    local reason="$1 (declined by .cursor/hooks/stage_commit_guard.sh — hand it to the user to run)"
