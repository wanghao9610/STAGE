...
- Runtime configuration lives in `.env`, created from `.env.example`: `STAR_HOME` (empty means standalone), `LATEX_ENGINE`, `ANON`, `STAGE_REPOSITORY`, `INVOLVE`, `STAGE_LANG`. Do not hardcode machine-specific paths.
