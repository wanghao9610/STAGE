...
把 Step 4 的检查从抽 5 条扩到**每一条**：按 index §4 那行记录的 URL 逐条重抓、逐字段比对，任何更正落盘之前先展示完整 diff——记录里已经不这么写的字段就按记录改正，绝不找理由解释过去。来源整个消失的条目保留原字段，在 index 的 §6 里添一行，并在摘要里点名。其余什么都不动：不新增条目、不重新聚类、不改笔记。这一遍的结果写进 index 的 §7。
