...
panel：五个委派，每批最多三个，按视角互不重叠——用`Agent` 子代理（`subagent_type: general-purpose`），每一批在同一条消息里一次派出去，这一批的评审员才是并行的。调用本 skill 本身就是在要它们；宿主根本不提供派发工具时走 `quick` 那条路，并在综合评审里写 `mode: quick`——一个从来就不独立的评审组，那才是它诚实的名字。每份简介包含它在 `references/review-dimensions.md` 里那一节的逐字文本、两份约定的逐字文本、摘要，以及范围那一行"只做这个视角；返回收集器约定的那些字段，除此之外什么都不返回"。每份简介另外带上这位评审员对某台 host 两次自己的请求之间等多少秒，以及它自己在运行目录下的缓存前缀（原则 3）——绝不带请求额度，礼貌不是用额度做成的。在 `high` 档，派发之前先说明切分方式（§6.8）。quick：主席把论文读一遍，自己按视角顺序填完五份收集器返回，魔鬼代言人放最后。
