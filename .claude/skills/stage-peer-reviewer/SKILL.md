...
description: >-
  ...
  runs /stage-peer-reviewer, or asks for a mock review, a pre-submission attack on the draft,
...
argument, ask which lens via AskUserQuestion, recommending one no `SIM_REVIEW_*` file in this
cycle covers yet.
...
and route to `/stage-stry-coach`. No subagents (§6): one lens is one reviewer's own read.
