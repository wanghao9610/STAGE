#!/usr/bin/env bash
# STAGE PreToolUse hook (Claude Code) — decline the git commands that break the
...
# normal permission flow. The other way, a `;`, `&`, `|`, or parenthesis inside
# a quoted message still ends a segment, so a message quoting a declined command
# after one is declined with the line; a heredoc message
...
deny() { # $1 = one-line reason
    printf '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"%s (declined by .claude/hooks/stage_commit_guard.sh — hand it to the user to run)"}}\n' "$1"
    exit 0
}
