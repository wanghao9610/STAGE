#!/usr/bin/env bash
# STAGE SessionStart and SubagentStart hook — inject the runtime-reported model id
...
# record. Every STAGE skill already runs its opening Bash calls from the project
# root, which is what makes the relative form resolve.
self=".claude/hooks/stage_model_id.sh"
