#!/usr/bin/env bash
# STAGE SessionStart hook — inject the project's memory index into session
# context so a session starts knowing what earlier sessions in this repository
# learned.
#
...
shared="$(entries "${root}/.stage/memory/MEMORY.md")"
machine="$(entries "${root}/.stage/memory/local/MEMORY.md")"
[ -n "${shared}${machine}" ] || exit 0

ctx="STAGE project memory — what earlier sessions in this repository learned, recorded under .stage/memory/ rather than in your own memory store. Each line is a pointer, not the fact: type · scope · last verified · file — summary. Open the file under .stage/memory/ before acting on one. A scope naming a machine or a cycle applies only there, and where a memory disagrees with a file in the repository, the file wins. A memory is never a source for a number, a venue rule, or what a cited paper says — those trace to mates/, venue.yml, and notes/refs/. Recording a new one: AGENTS.md section 10."
[ -n "${shared}" ] && ctx="${ctx}
Shared (.stage/memory/):
${shared}"
[ -n "${machine}" ] && ctx="${ctx}
Machine-local (.stage/memory/local/):
${machine}"
