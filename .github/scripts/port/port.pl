...
for my $tree (@trees) {
    ...
    for my $rel (@rels) {
        ...
        # A file new to .agents/skills starts from its source frontmatter, so
        # --write creates it; a new SKILL.md stays a hand job, because each
        # harness owns its own frontmatter.
        if (!defined $current && $mode eq 'write' && $rel !~ m{(?:\A|/)SKILL\.md\z}) {
            ($current) = split_fm($SOURCE{$rel});
        }
        if (!defined $current) {
            $missing++;
        ...
        my $errors_before = scalar @errors;
        my $generated = generate(
            $rel, $tree, $adapters, \@errors,
            $mode eq 'regen' ? $tree : undef
        );
        # An override that lost its anchor leaves the file half adapted:
        # --write keeps what the tree holds, and --regen records nothing.
        next if $mode ne 'check' && @errors > $errors_before;
    ...
    if ($mode eq 'regen' && @errors) {
        print "      ! $_\n" for @errors;
        print "      .$tree overrides left unchanged: fix the anchors above first\n";
        $bad = 1;
        next;
    }
    if ($mode eq 'regen') {
