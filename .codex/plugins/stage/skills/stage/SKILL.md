...
Do not reproduce a selected skill's workflow from this router. For an unmarked skill, load and follow that `stage-*` skill from the current project's available skills. For a skill marked `†`, request the confirmation required by the roster, show the exact `$stage-<name> <argument>` invocation, and wait.
