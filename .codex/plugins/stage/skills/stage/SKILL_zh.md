...
- `$stage` 是本通用路由器。
- 当名册写作 `/stage-<name> <argument>` 时，以 `$stage-<name> <argument>` 调用选中的项目 skill。

不要在本路由器中复述已选 skill 的工作流。
对于未标记的 skill，加载并遵循当前项目可用的对应 `stage-*` skill。
对于标有 `†` 的 skill，按名册要求请求确认，显示准确的 `$stage-<name> <argument>` 调用，然后等待。
