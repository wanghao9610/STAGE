#!/usr/bin/env bash
# STAGE PreToolUse hook (Codex CLI) — decline the git commands that break the
...
deny() { # $1 = one-line reason
    printf '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"%s (declined by .codex/hooks/stage_commit_guard.sh — hand it to the user to run)"}}\n' "$1"
    exit 0
}
