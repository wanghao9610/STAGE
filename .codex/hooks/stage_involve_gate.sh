#!/usr/bin/env bash
# STAGE PermissionRequest hook (Codex CLI) — skip the approval prompt for a file
# edit while the project runs at INVOLVE=low (.env, writing workflow conventions
# §7.7). Confirmation points are untouched: the STOP line, deletions and
# overwrites, and every venue.yml value entering as confirmed are questions a
