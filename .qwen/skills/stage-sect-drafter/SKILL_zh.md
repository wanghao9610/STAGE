...
**通用规约。** 动手之前先读 `docs/mds/stage-workflow/writing-workflow-conventions.md`（中文对照：`writing-workflow-conventions.zh-CN.md`）——整份读，每次运行开始时；不做分节选读。它通过自己的 `read_file` 调用到达，绝不被 `cat` 进一条 `run_shell_command` 命令。它是所有 STAGE skill 共享的基线；对本 skill 约束最紧的是 §5（章节与周期解析）、§8（产物登记表及其过期规则）、§9（编造边界）。本文件只写本 skill 特有的部分，更严处以本文件为准。
...
1. 读规约文件（整份，用它自己的 `read_file` 调用），然后读 `notes/story.md`（pitch、当前 `cycle:`）、`notes/outline.md`、`notes/claims.md`、`notes/notation.md`，以及存在时的 `notes/style.md`（原则 7）。
2. 整份读取自然写作指南，用它自己的 file read 调用。
3. 缺 story 或缺提纲，意味着流水线还没到起草的时候：停下并路由到 `stage-stry-coach` 或 `stage-outl-planner`，而不是临场编一个结构出来。

### Step 1：解析章节

1. 按 §5 对着 Sections 表解释 `SECTION`：编号、文件 slug 或标题匹配。缺失或有歧义 → 列出各行及状态，经 `ask_user_question` 提问（不可用时用纯文本）。
