...
description: >-
  ...
  checkbox under tasks/, and mark conceded claims weakened. Use when the user runs /stage-resp-writer,
  or asks to draft a rebuttal or response letter, answer reviewers point by point, or decide what to
  concede. Never edits the manuscript or the review files.
argument-hint: "[CYCLE] [DESCRIPTION] [involve=high]"
---
...
3. **Three dispositions; the costly ones are user-owned.** rebut — evidence in hand, cite it;
   ...
   point at a time via `ask_user_question` (§7); evidence-backed rebuttals may proceed and are
