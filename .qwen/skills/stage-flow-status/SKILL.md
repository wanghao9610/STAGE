...
1. **One load, then reason.** Everything this skill reads arrives in a single message — five `run_shell_command`
   ...
   limit, and a `run_shell_command` result past roughly 30 KB is written out to a file that costs a round trip to
