...
- Every question — an unrecognized token or a missing `--source` (Step 0), the import-rewrite confirmation (Step 1), a name collision, a changed checksum, and the provenance question (Step 2), an obsolete file — goes via `ask_user_question`, one question per call. If it is unavailable (headless / scripted), fall back to plain text and require an explicit answer before the write.
