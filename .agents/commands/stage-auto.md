...
4. After each run ends, take the next action it names, through the goal filter, steps 6 and 7, and step 2. This loop takes it, never the run that named it, even where the goal's words ask for more (conventions §11.4), so nothing is started twice. Where it names none, run `stage-flow-status` again.
