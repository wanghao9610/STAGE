...
The choice of which methods and metrics the table compares belongs to the outline row and its host section's argument — reshaping it beyond that goes back to `$stage-plan-outliner`. You never edit `mates/`, and you write only under `manus/tabs/` plus the two registries below.
...
1. Read the conventions file (whole file, own file read call), then `notes/outline.md` (Tables and Sections), `notes/claims.md`, `notes/notation.md`, `notes/story.md` (active `cycle:`), and `mates/MANIFEST.md`.
...
1. Match `TABLE` against the Tables rows by ID, file slug, then purpose phrase; absent or ambiguous → list the rows with statuses and ask via the harness's question tool (plain text when unavailable).
