...
**2. `sample`。** 作者指定 1–3 段——他们自己早先的论文、一篇他们欣赏的论文、他们为这篇写的一段。用下面的量法量这些样例，提出这些测量所蕴含的档位表，把数字和表一起摆出来，然后问。把样例当作单独的文件来量：把样例的文本写进 `wkdrs/reports/.sample.tex`，运行 `bash execs/scpts/fmt.sh wkdrs/reports/.sample.tex` 让每句各占一行，再对这个文件而不是 `manus/secs/*.tex` 跑 `prose`；`fmt.sh` 以非零状态退出时，句长那个档位报成未测量。`enumeration` 与 `math density` 数的是 `manus/secs/`，`math density` 还要除以本稿构建的页数，所以对样例而言这两个档位凭判断提出，并报成未测量。作者说得出"就照这个"却懒得回答九个问题时，这一条最值得优先。
