...
description: >-
  ...
  \bibliography 的改动都在计划里先点名再施行，接好的树用一次树外构建验证。外部草稿是复制进来的，
  它的源码树绝不被修改。正文里已有的每个数字都记为一条 unsourced 主张——这就是
  $stage-clms-auditor 吸收进主张台账并逐条清掉的待办——候选证据文件被路由给 $stage-evid-curator，
  在这里绝不复制进 mates/。只要用户调用 $stage-proj-adopt、想把一篇已有论文、学位论文章节或
...
按用户的语言回复：中文对话用中文答。接入记录一律用英文写——每个下游 skill 都要读它——其中路径、指标名与被引用的主张按草稿本来的语言逐字保留；对话是中文时，收尾的聊天摘要用中文给出。
...
一次问一个结构化问题——宿主有提问工具就走它，否则用纯文本——只问探测定不下来的事：哪个候选是主文件、哪些目录是正文 / 证据 / 垃圾、是否存在配对的 STAR 仓库（它来填 `STAR_HOME`）、以及没有任何东西点名 venue 时的目标 venue。选项来自探测结果，并标出推荐。这个点通过之前什么都不写。
...
4. 批准：一个结构化问题——全部批准、按组批准（章节 / 图 / 表 / bib / 样式）、或者中止。未获批准的行不动。
...
扫描接进来的正文与表格：每一个作为结果呈现的数字——一个指标值、一个百分比、一句"提升了"、一次加速、一个表格单元格——变成一条待办行：逐字的主张、它搬迁后的 `file:line`、以及正文有所暗示时那个被怀疑的证据来源（一次 STAR 运行、一篇被引论文、某位合作者的文件），否则写 `unknown`。设置类数字不算——超参、公式常数、引文年份——拿不准就算进来：多一行只花一次核对，漏一行赔上论文的可信度。这份待办记在接入记录里，不写进台账：`notes/claims.md` 只有一个写入者，`$stage-clms-auditor`，它把每一行作为一条 unsourced 主张吸收进去并逐条清掉。
...
- 这里绝不写：`mates/**`（只有两个写入者：`$stage-evid-curator` 与 `execs/scpts/import.sh`）、`notes/claims.md`（`$stage-clms-auditor` 的台账）、`notes/refs/**`、`cycls/**`，以及任何外部 `SRC_PATH` 树。构建产物经 `execs/run.sh` 落到 `wkdrs/builds/` 下，那是它的地盘。
...
- 两个确认点都走宿主的提问工具，一次调用一个问题。没有提问工具可用时（headless / 脚本化），回落到纯文本——依然一次一个，依然要求在任何写入之前给出明确答复。
