...
description: >-
  ...
  prose logged as an unsourced claim. Use when the user invokes stage-proj-adopt, or wants to bring an
...
4. Approval: one question via your question tool — approve all, approve by group (sections / figures / tables / bibs / styles), or abort. Unapproved rows do not move.
...
- Both confirmation points go through your question tool, one question per call. If it is unavailable (headless / scripted), fall back to plain text — still one at a time, still requiring an explicit answer before any write.
