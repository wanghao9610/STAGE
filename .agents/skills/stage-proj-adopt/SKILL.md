...
4. Approval: one structured question — approve all, approve by group (sections / figures / tables / bibs / styles), or abort. Unapproved rows do not move.
...
- Both confirmation points go through the harness's question tool, one question per call. If no question tool is available (headless / scripted), fall back to plain text — still one at a time, still requiring an explicit answer before any write.
