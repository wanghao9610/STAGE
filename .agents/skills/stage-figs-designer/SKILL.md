...
2. **The local source of truth is PPTX.** Every figure created or substantively revised with that pipeline has a one-slide `manus/figs/srcs/<slug>.pptx`; its row's Source names that file, and `manus/figs/<slug>.pdf` is exported from it. Preserve legacy TikZ, Python, and Draw.io files during audits; while the pipeline is available, do not create a new legacy-only source. Artwork produced outside the repository may instead use the existing `mates/MANIFEST.md` origin after `stage-evid-curator` registers it. A PDF with neither origin is an orphan.
...
4. **Plotted numbers remain traced (§9a).** Before composing, create `manus/figs/srcs/<slug>.sources.md`. Give every series and every numeric or comparative caption claim its own line containing `% src: mates/<...>#<anchor> sha256: <hex>`, the hex copied from that entry's `sha256:` in `mates/MANIFEST.md` as read this run; no blanket source line covers several series. Mirror the same element-to-anchor mapping in a `[Sources]` block in the PPTX speaker notes. A value absent from fingerprinted evidence is omitted and handed to `stage-sect-drafter` as `\todo{...}` caption text; no remembered point, interpolation, or plausible curve is allowed.
...
9. **Fan out per figure (§6).** A no-argument audit with more than 6 Figures rows dispatches one read-only sub-agent per figure on the READ tier's model (conventions §11.6), where the harness can name one; each reads only that row, its named files, and the MANIFEST entries they cite, then returns the origin verdict, source-map verdict, fingerprint verdict, render-hash verdict, and nothing else. A build does not split: one invocation builds one figure (conventions §11.3). The main agent alone writes `notes/outline.md`, renders and visually checks the integrated figure, runs repository gates, talks to the user, and offers the commit.

## Workflow

**Where this run executes.** This run's tier follows its mode: the no-argument audit is READ; `plan`, `teaser`, and a figure build are EXEC (conventions §11.6). It stays in the session that started it, on the session's model. When that tier's value (`STAGE_READ_MODEL` or `STAGE_EXEC_MODEL`) names a model that is not an alias of the session's, say so in one line once the mode is resolved — the tier, that model, and the one way to get it: switch the session's model — then continue here.

### Step 0: Load

Read the conventions (whole file), then `notes/outline.md`, `notes/story.md`, `notes/notation.md`, `notes/claims.md`, `mates/MANIFEST.md`, and the active cycle's `tasks/<cycle>_promises.md` when it exists (Step 7.1); list `manus/figs/` and `manus/figs/srcs/`. Resolve `LATEX_ENGINE`, `ANON`, `STAGE_LANG`, and `INVOLVE` from `.env`. No `notes/outline.md` → say so and route to `stage-outl-planner`; the Figures table lives there, so stop.

For a figure build or revision, check for the local figure pipeline: the installed `Presentations` skill with `@oai/artifact-tool` to compose, `soffice` resolved by the workspace dependency loader to render, and `slides_test.py`, `render_slides.py`, and `view_image` to check; `image_gen` is optional. With the composer present, load that skill and its required PowerPoint style/API instructions, then call the workspace dependency loader; a missing renderer or checker is Step 5's degraded check. Without the composer, say so and take the source path: tikz for architecture and diagrams, python for data plots, or drawio for flowcharts at `manus/figs/srcs/<slug>.*`, each data series carrying its Principle 4 `% src:` line in that file (`# src:` in python); tikz compiles standalone with the `.env` engine into `wkdrs/builds/figs/` and its PDF is copied to `manus/figs/<slug>.pdf`, python runs from the repository root and writes that PDF itself, and a drawio export is handed to the user as an exact step with the row held at `draft` until the PDF lands. That path runs Steps 3.1–3.3 with those `src:` lines in place of a `.sources.md`, Step 5's build and lint gates, Step 6 for the teaser with items 7–8 not applicable, and Step 7 with the source file in place of the PPTX, and writes no `.render.yml`; a row whose Source is already a `.pptx` is not rebuilt in another form: say that it needs the pipeline, name `stage-figs-designer <ID>` for a harness that carries it, and stop. Treat the paper's figure brief as explicit custom visual direction: do not apply a generic slide-deck template or expose slide-planning language in the figure.

### Step 1: Resolve the mode

First match wins: `plan` → Step 2; `teaser` → the teaser, Steps 3–6; a figure token → that figure (§5 matching; ambiguity → ask, §7), Steps 3–5; `plan`, `teaser`, and a figure run all end with Step 7 (`plan`: 7.2–7.3 only); no argument → audit:

1. Resolve every `manus/figs/*.pdf` to either a local editable source or a `mates/MANIFEST.md` entry; headline every orphan.
2. For a figure whose Source names a `.pptx`, require `<slug>.pptx`, `<slug>.sources.md`, and `<slug>.render.yml`; compare the recorded SHA-256 values to the current PPTX and PDF rather than mtimes. A row whose Source names a tikz, python, or drawio file is a legacy origin (item 4) even when an older `.pptx` and `.render.yml` sit beside it: report that leftover pair rather than calling the figure stale.
3. Check every Figures row against disk: File exists or Status is `planned`; Source resolves; a `mates/` Source still has its MANIFEST entry; a local source map has one `% src:` entry per data series and claim-bearing caption sentence, and each `sha256:` its source map recorded (`<slug>.sources.md`, or the `src:` comments of a tikz, python, or drawio source) still equals that MANIFEST entry's current `sha256:` — a mismatch means the evidence was replaced after the build, so set that row's Status to `draft` in `notes/outline.md` unless it already stands lower, with its provenance fields (§8) and the run's one commit (§1), and name `stage-figs-designer <ID>` as its rebuild; an entry recording none is reported as unchecked.
4. Treat older TikZ, Python, or Draw.io sources as valid legacy origins, but propose the PPTX path, where the pipeline is available (Step 0), when that figure next needs substantive revision. Do not rewrite them during an audit.
5. Scan drafted sections for `\includegraphics` files no row plans; propose a row rather than adopting it silently.
6. Report drift, any row item 3 demoted, and one next action with its exact command; beyond item 3's Status write and its commit (§1.6), go no further unless asked.
...
- `manus/figs/srcs/<slug>.*` — the tikz, python, or drawio source a build without the pipeline writes (Step 0), a `% src:` line on every data series.
...
- `tasks/<cycle>_promises.md` — the boxes this revision keeps, ticked on the author's yes (Step 7.1).
