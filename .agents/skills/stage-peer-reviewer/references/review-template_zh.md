...
带 `extern=` 的运行写出的报告。正文就是上面的综合评审，只有四处改动，此外没有别的：

- frontmatter 写成 `type: referee_report`、`target: <extern= 拿到的那个路径>`、`venue:`、`scale:`、`mode:`、`generated:`、`recommendation:`——不带 `cycle:` 或 `commit:`，因为根本没有。
