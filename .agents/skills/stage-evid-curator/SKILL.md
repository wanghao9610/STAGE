...
2. A diff that rewrites already-registered entries is confirmed first via one structured question — through the harness's question tool where it has one, else plain text — naming each entry and the numbers that move: a refreshed number can silently contradict a sentence the draft already typeset (Principle 6). New-only imports proceed without a question.
...
3. One structured question for what no probe can know: `source` — where this came from (path, URL, or person) — and `shows`, skipped when the user's request already said both.
...
- Two asks exist — the import-rewrite confirmation (Step 1) and the registration provenance question (Step 2) — through the harness's question tool, one question per call. If no question tool is available (headless / scripted), fall back to plain text and require an explicit answer before the write.
