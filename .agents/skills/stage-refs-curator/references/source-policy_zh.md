...
**它写一个仓库文件：任务书点名的那份笔记。** `notes/refs/<ABBREV>.md`，此外不写别的——一篇论文、一个文件、一个写入者（规约 §6.2）。文件名由任务书指定，不在这里挑，因为"它在 `notes/refs/` 里是否唯一"只有主 agent 看得见。这份笔记带 `## What it does`、`## Citable facts` 与 `## Relation to ours`，最后这一节对着任务书给出的故事与主张记录表写。它不带 `bibkey:`，不带 `added:`，不带索引行，也不带影响力分：那些住在 `reference.bib` 与 `refs_index.md` 里，是只有一个写入者的单个文件。它的 frontmatter 倒要带 `model_id:` 和一条 `model_trail:` 记录，而这个 id 是你自己的，绝不是会话的（规约 §8）：在写的那一刻，对着你自己的记录，跑你自己的启动行注入的那条命令，照抄它的输出；你的上下文若提供写后检查，就对这份笔记跑一遍。
