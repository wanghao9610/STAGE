...
**它写一个仓库文件：任务书点名的那份笔记。** `notes/refs/<ABBREV>.md`，此外不写别的——一篇论文、一个文件、一个写入者（规约 §6.2）。文件名由任务书指定，不在这里挑，因为"它在 `notes/refs/` 里是否唯一"只有主 agent 看得见。这份笔记带 `## What it does`、`## Citable facts` 与 `## Relation to ours`，最后这一节对着任务书给出的故事与主张记录表写。它不带 `bibkey:`，不带 `added:`，不带索引行，也不带影响力分：那些住在 `reference.bib` 与 `refs_index.md` 里，是只有一个写入者的单个文件。
...
**主 agent 拿这些返回做什么。** 逐篇处理那三样没有委派者会打开的东西：`refs_index.md` 那一行、bib 条目上方的 `% src:` 出处行、以及影响力分。`floor_evidence: not reached` 与正文压根抓不到是同一个结果——bib 条目保留，没有笔记。一份声称够到了底线却报不出 `note_path` 的返回，或者 `facts` 大于零而那份笔记并不在磁盘上的返回，是自相矛盾的，那篇论文在这里重读一遍（规约 §6.3）。返回里的任何东西都绝不进 `reference.bib`：那里的字段来自书目记录，不来自论文自己的页面。
