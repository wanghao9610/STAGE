...
> 本文件是 `source-policy.md` 的中文对照版，随英文版同步维护，供人阅读；运行时装载的仍是英文版。两版冲突时，以 `source-policy.md` 为准。

`manus/bibs/reference.bib` 里的每个字段，要么追溯到一条抓回的记录——不论是哪次运行抓的，条目的 `% src:` 行和它在 index 里的那一行都写明记录 URL 与抓取日期——要么来自从 `mates/` 播种过来的上游条目；用户手工添加的条目则标为 `% src: user-supplied`。本文件固定记录可以从哪来、按什么顺序来、一条记录如何判定属于某篇论文、转录之后允许做哪些改动，以及每个 citekey 与出处行的形状。第一次抓取之前先读它。
...
**查询。** 由检索画像生成，差异要体现在种类上而不是措辞上，而这些种类就是下界：任务本身的词、机制的词、这个领域实际在用的同义说法（一条只讲你自己词汇的查询，只找得到和你共用词汇的论文）、benchmark 与数据集名字，以及论文给自己取标题时那种"X for Y"的形状——每个适用于本文的种类各出一条，不因为"看着没戏"就省掉哪一条。上界是饱和而不是条数，而且只在下界满足之后才看——此前查空的一条照记、扫掠继续；过了下界，每条新查询都是某个种类的又一个实例，绝不是换个措辞，只要一条查询还能带回前面没出现过的候选就继续，直到某一条回来全是已见过的，这一轮扫掠就结束。每条查询都记下它返回了多少条命中——命中为零的也记，因为"查了，什么都没有"和"根本没查"是两种状态，只有其中一种构成停下来的理由。这份记录同时让停止条件事后可核：查干了的扫掠看得出来，早停的也看得出来。

**终止。** 这里没有任何东西靠请求额度来配给。额度会是一个谁也辩护不了、谁也没法拿日志去核的数字；取而代之，每个机制各自带一个停止条件，而每一个都看得见地写在这轮运行留下的记录里。扫掠在过了下界之后、某条查询回来全是已见过的时候停。扩展停，是因为它的输入在开跑之前就已经闭合。作者挑完之后的部分，在挑中的论文都收完时停——挑了几篇是作者的决定，不是本文件配给的额度。没有哪一步可以一直跑下去，也没有哪一步需要把计数器递给下一步。礼貌是另一个问题、另一种东西——它是速率，由下面的分主机速率结清，绝不由总量结清。某个机制停下来时要点名是哪个条件让它停的：查干了、前沿走完了，还是主机拒绝了。绝不悄悄拿记忆把剩下的补齐。
...
- **发表分**——抓回记录的 venue 字段对 `venue-tiers.md` 的档位表匹配：旗舰 `10`，二档 `7`，其他正式发表 `4`，仅预印本 `2`。
...
分档与权重按 CS/AI 文献校准，也正因如此放在这个文件里——换领域的项目改的是这些常数。常数可改——改在 STAGE 上游或 `STAGE_REPOSITORY` 指向的 fork 里再同步，因为 `execs/update.sh` 会替换论文仓库里的这个文件；"印象不入分"不可改。
...
- **发表档**是离线的：抓回记录的 venue 字段对 `venue-tiers.md`。
...
- **阅读扇出拆的是速率，不是额度**（规约 §6.9）。每份任务书都用秒数写明：这个委派者对某台 host 两次自己的请求之间等多久——上面那个间隔乘以这次扇出最多同时在跑几个委派者，这个宽度在第一个委派者派出之前就定下。三个同时读 arXiv 页面的委派者各等 9 秒，于是整轮运行对 arXiv 仍然是每 3 秒一页。没有固定上限——宽度和委派者派出的先后都由主 agent 定，任务书带着它定出来的那个数——而这一页上除了委派者自己那篇论文的页面之外，每一次抓取都是主 agent 的。
- HTTP 429 / 503 → 指数退避（2s、4s、8s），最多重试 3 次，然后放过并记下失败。限速永远不是凭记忆补缺口的理由。
- 某个来源什么都没返回，记成"not found in `<source>`"——那是一次抓取的结果，不是这篇论文不存在的证据。

## 阅读收集者合同

阅读那一步扇出时，一个委派者做什么、返回什么（`SKILL.md` 原则 9）。一篇论文一个。委派者自己去抓那篇论文自己的页面——规约 §6.9——两次自己的请求之间按任务书写明的秒数等待，每份原始内容都先缓存到任务书点名的前缀下再读。别的 URL 一概不开：不抓书目记录，不打检索端点，不调 GitHub API，这些都留在主 agent 手里。

**它写一个仓库文件：任务书点名的那份笔记。** `notes/refs/<ABBREV>.md`，此外不写别的——一篇论文、一个文件、一个写入者（规约 §6.2）。文件名由任务书指定，不在这里挑，因为"它在 `notes/refs/` 里是否唯一"只有主 agent 看得见。这份笔记带 `## What it does`、`## Citable facts` 与 `## Relation to ours`，最后这一节对着任务书给出的故事与主张记录表写。它的 frontmatter 按规约 §8.7：`title:`、`venue:`、`year:`、`bibkey:` 与 `added:`，照任务书给出的值原样抄。它不带索引行，也不带影响力分：那些住在 `refs_index.md` 里，是只有一个写入者的单个文件。它的 frontmatter 还要带 `model_id:` 和一条 `model_trail:` 记录，而这个 id 是你自己的，绝不是会话的（规约 §8）：在写的那一刻，对着你自己的记录，跑你自己的启动行注入的那条命令，照抄它的输出——那行直接写着 id 时就抄那个 id；没有这行时抄你自己系统提示里写的 id，都没有就写 `unrecorded`，绝不抄会话的 id，也绝不猜；你的上下文若提供写后检查，就对这份笔记跑一遍。

**§9b 约束的是写下那条要点的人，在这里就是你。** 一条 `## Citable facts` 就是一句还没写出来的手稿断言，而 `stage-cite-auditor` 日后要只凭你这份笔记、不重新打开论文，就对一句手稿下判定。所以每条要点都是一个自足的事实——数字要带着它的数据集、指标与设置一起走——点名它出自哪一节、哪张表、哪个公式，并带上从缓存页面里逐字符抄下来的至多 25 个词。你没法钉到本次运行读过的东西上的事实，不进这个文件；而把转述当成引文写下，正是这条规则要挡的那种失败。

然后返回下面这些字段，此外什么都不返回：

- `note_path`——你写出来的那个文件，或者 `none`，理由写进 `failures`。
- `abbrev`——任务书指定给你的那个代号，原样回传。
- `facts`——这份笔记带了多少条 `## Citable facts` 要点。
- `floor_evidence`——`{sections_reached: [...], results_table: <表题加一行，逐字>}`，或者 `not reached`。底线是摘要、intro、方法与主结果表；确实没有结果表的论文在这里说明，并点名什么顶了它的位置。`not reached` 意味着你没写出笔记——在 `note_path` 里说明，那条 bib 条目就单独留着。
- `repo_named`——这篇论文自己的页面挂出来的仓库，或者 `none found`。你不去抓它；那一次 GitHub 调用是主 agent 的。
- `pages_fetched`——`[{url, cache_path, status}]`，你发过的每个请求一行，按发出的顺序。
- `failures`——`[{what, why, host, retries}]`：加载不出来的页面、抓回的页面里没有的那一节、解析不出来的表、抽不出文字的扫描件。
- `model_id`——你写进笔记的那个 id；没写出笔记时为 `none`。

**主 agent 拿这些返回做什么。** 逐篇处理那三样没有委派者会打开的东西：`refs_index.md` 那一行、bib 条目上方的 `% src:` 出处行、以及影响力分。`floor_evidence: not reached` 与正文压根抓不到是同一个结果——bib 条目保留，没有笔记。一份声称够到了底线却报不出 `note_path` 的返回，或者 `facts` 大于零而那份笔记并不在磁盘上的返回，是自相矛盾的，那篇论文在这里重读一遍（规约 §6.3）。返回里的任何东西都绝不进 `reference.bib`：那里的字段来自书目记录，不来自论文自己的页面。每份返回的 `model_id` 连同这次扇出的档位，记进本次运行的决策记录（规约 §11.6）；笔记自己的 `model_id` 绝不拿会话的去覆盖。

## 收尾前的自查

1. 每条抓回的条目，不论是哪次运行抓的，都在 `refs_index.md` 里有一行出处**且**条目上方那行 `% src:` 与该行的 URL 和日期一致；本次运行抓回的条目还要在本次运行的 `wkdrs/refs_<date>/raw/` 下有缓存的原始内容，更早运行的缓存不去找，因为 `wkdrs/` 从不提交。播种来的条目上方写 `% src: mates/<slug>/metds/refs/reference.bib (seeded YYYY-MM-DD)`，它那一行来源写 `mates/<slug>`、没有记录 URL、日期是播种日期；用户手工添加的条目上方写 `% src: user-supplied`，它那一行来源写 `user-supplied`，既没有 URL 也没有日期。
2. 从本次运行抓回的条目里随机重抓 5 条（`verify` 模式下按类别查全部——见 SKILL.md Step 6a），与文件逐字段 diff。有出入 → 把文件改成与来源一致（`verify` 模式下须先给用户看 diff 并得到同意），并重查该条所在的整批。
