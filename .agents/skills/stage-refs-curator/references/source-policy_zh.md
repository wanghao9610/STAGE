...
- **阅读扇出拆的是速率，不是额度**（规约 §6.9）。每份任务书都用秒数写明：这个委派者对某台 host 两次自己的请求之间等多久——上面那个间隔乘以这次扇出派出了多少个委派者。三个读 arXiv 页面的委派者各等 9 秒，于是整轮运行对 arXiv 仍然是每 3 秒一页。没有固定上限——宽度是主 agent 定的，任务书带着它定出来的那个数——而这一页上除了委派者自己那篇论文的页面之外，每一次抓取都是主 agent 的。
