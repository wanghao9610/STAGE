...
description: >-
  ...
  fetchable record is listed for manual check, never guessed into the bib. Use when the user
  invokes $stage-refs-curator, or asks to add a reference or reading note, clean or deduplicate the
...
4. Report the cluster map and thin clusters as a read-next list (one `$stage-refs-curator add …`); drafting Related Work from it is `$stage-sect-drafter`'s.
...
2. Digest in chat: entries added / seeded / failed / needs-manual-check, notes written (`ABBREV` → file), hygiene fixes, the cluster map or read-next list, and routing — verify manuscript assertions → `$stage-cite-auditor`; draft Related Work → `$stage-sect-drafter`; import an upstream refs tree → `$stage-evid-curator`.
