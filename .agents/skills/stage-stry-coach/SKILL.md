...
Work the schema order (conventions §8), each section drafted → shown → confirmed via the `request_user_input` tool ("write it" / "needs edits") → written, `updated:` refreshed; close each boundary in 1–2 sentences — what settled, what the next section opens:
