...
1. Opening load, one message where possible: the conventions file; `notes/story.md`, `notes/claims.md`, and `notes/adopt.md` where present; `mates/MANIFEST.md`; one shell call for `date +%F` (real dates, conventions §4) plus a listing of `mates/` and `cycls/`.
