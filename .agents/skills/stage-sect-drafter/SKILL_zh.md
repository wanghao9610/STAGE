...
**通用规约。** 动手之前先读 `docs/mds/stage-workflow/writing-workflow-conventions.md`（中文对照：`writing-workflow-conventions.zh-CN.md`）——整份读，每次运行开始时；不做分节选读。它通过自己的 file read 调用到达，绝不被 `cat` 进一条 shell 命令。它是所有 STAGE skill 共享的基线；对本 skill 约束最紧的是 §5（章节与周期解析）、§8（产物登记表及其过期规则）、§9（编造边界）。本文件只写本 skill 特有的部分，更严处以本文件为准。

**自然写作指南。** 每次运行开始时还要整份读取 `docs/mds/stage-workflow/human-writing-guide.md`（中文对照：`human-writing-guide.zh-CN.md`），用它自己的 file read 调用。它定义本 skill 使用的、受证据约束的自然写作检查；本 skill 更严格的证据与记录表规则仍然优先。
...
7. **盘上有作者的行文档案，就照它写。** `notes/style.md`（§8.11）是文风档案——句长与节奏、语态、过渡方式、限定语的分量、罗列的形式，以及这篇论文不用的那些词——一份草稿照着它来。它压不过任何东西：§9 最先，其次是记号规范（原则 4），再次是 venue 的格式，最后才是这份档案。所以没有任何档位能授权一个数字（原则 1），也不能授权一条证据扛不住的主张（原则 3），而 `hedging: minimal` 收紧的是措辞，绝不撤掉证据所要求的限定语——主张的强度归记录表，不归偏好。没有档案时采用克制、直接的学术默认：先给出具体主张，技术术语保持稳定，句长只随论证需要变化。绝不发明一份档案，也绝不去写那个文件——它归 `stage-copy-editor style`。
...
1. 读规约文件（整份，用它自己的 file read 调用），然后读 `notes/story.md`（pitch、当前 `cycle:`）、`notes/outline.md`、`notes/claims.md`、`notes/notation.md`，以及存在时的 `notes/style.md`（原则 7）。
2. 整份读取自然写作指南，用它自己的 file read 调用。
3. 缺 story 或缺提纲，意味着流水线还没到起草的时候：停下并路由到 `stage-stry-coach` 或 `stage-outl-planner`，而不是临场编一个结构出来。

### Step 1：解析章节

1. 按 §5 对着 Sections 表解释 `SECTION`：编号、文件 slug 或标题匹配。缺失或有歧义 → 列出各行及状态，经 your question tool 提问（不可用时用纯文本）。
...
7. 每段只承担一个任务：具体主张、证据或推理、再说明它与本节问题的关系。不要用尚未被本段内容支撑的意义宣告开头，不要为固定技术术语轮换同义词，也不要在证据结束后再接一句泛化的重要性结尾。
8. 离开本节之前，按自然写作指南做一次段落级检查。判断共同出现的模式及其修辞作用，不盯着孤立词语；围绕主要主张重写整段，不做逐词替换。把结果与修改前的数字、引用与交叉引用 key、`% src:` 锚点、`\todo{}` 标记、主张强度、归属和必要限定语逐一对照。凡是无法证明这些受保护内容未变的地方，恢复原文或只报告问题。
...
1. 提议跑一次 `execs/run.sh` 构建——草稿不得弄坏编译。对本节 grep `\todo{`，报出计数与每条 todo 的文字；构建成功后跑 `execs/scpts/lint.sh`。其中的散文模式发现只供人工复核；证据、引用、匿名与 `\todo` 的失败仍保持原有约束力。
