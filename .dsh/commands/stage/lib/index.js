// Zero-dependency `/stage` slash command. It is a thin front door: the handler
// injects one follow-up turn that reads the shared router
// (`.agents/commands/stage.md`) and applies it to the user's request, so the
// model routes to the right STAGE skill and executes it. The router owns the
// routing table and the "empty request selects stage-flow-status" rule; this
    ...
    handler: ({ agent, rawInput }) => {
      ...
      agent.followup({
        ...
        content: [{
          type: "text",
          text: request === ""
            ? "Read `.agents/commands/stage.md` and apply its router to an empty request (select `stage-flow-status`)."
