...
2. A diff that rewrites already-registered entries is confirmed first with one question via ask_user_question, naming each entry and the numbers that move — a refreshed number can silently contradict a sentence the draft already typeset (Principle 6). New-only imports proceed without a question.
...
- Two asks exist — the import-rewrite confirmation (Step 1) and the registration provenance question (Step 2) — via ask_user_question, one question per call. If it is unavailable (non-interactive `dsh --profile headless`, no human to answer), fall back to plain text and require an explicit answer before the write.
