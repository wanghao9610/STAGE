...
6. **A refresh is a change to the paper.** Re-import can move a number the draft already quotes. Rewrites of registered entries are therefore confirmed before they run, and refreshed entries always route to `stage-tabs-builder` (its tables just went stale), `stage-clms-auditor` (claims citing them need re-checking), and, when an entry's `sha256:` moved, `stage-figs-designer` with no argument (its audit demotes every figure whose source recorded the old checksum). Evidence drifting under finished sentences is how drafts become fiction.

7. **Fan out the `check` reconciliation (§6).** More than 3 slugs under `mates/` → one delegate per `mates/<slug>` tree, on the READ tier's model (conventions §11.6), where the harness can name one, each grading its own files `ok | unregistered | missing | tampered | unchecked` against their manifest entries and returning that table and nothing else. Nothing it touches is written: `mates/` is read-only to every agent in this run, delegates included (§6.4, §10). What stays here is what a split would break — the staleness diff is one `import.sh --diff` call, which walks every imported slug itself, and registration is a confirmation point, the file read in full and its provenance asked before an entry exists (§6.5).

## Workflow

**Where this run executes.** This run's tier follows its mode: `check`, which a bare invocation runs, is READ; `import` and `register` are EXEC (conventions §11.6). It stays in the session that started it, on the session's model. When that tier's value (`STAGE_READ_MODEL` or `STAGE_EXEC_MODEL`) names a model that is not an alias of the session's, say so in one line once the mode is resolved — the tier, that model, and the one way to get it: switch the session's model — then continue here.
...
Read `.env`. `STAR_HOME` set and pointing at a real repo → the star side is live; empty or missing → `import` needs `--source PATH`, asked for when the invocation names none (conventions §3.2), and star staleness reaches only the slugs whose recorded `source:` is a path this machine has (a `$STAR_HOME/` one reads `unknown`), both said up front; manual evidence still works in full — the pairing is optional by design. Resolve the mode from the first token; none → `check`.

### Step 1: `import` — pull or refresh star evidence

1. Preview: `bash execs/scpts/import.sh --diff` with the same `--source`/`--slug` step 3 will pass, and `--source` set to the `STAR_HOME` path when the user named none, since a bare `--diff` walks every imported slug instead — a byte comparison of upstream's working tree against each copy under `mates/`, writing nothing. It also names a dirty upstream: importable files with uncommitted changes, which the import would pin as `<sha>-dirty`.
2. A diff that rewrites already-registered entries is confirmed first with one question via ask_user_question, naming each entry and the numbers that move — a refreshed number can silently contradict a sentence the draft already typeset (Principle 6). A diff with only `new upstream` lines proceeds without a question; the script imports a slug whole, so it still re-stamps that slug's existing entries with the new `source-commit:` and `imported:`, their bytes and `sha256:` unchanged. A declined confirmation imports nothing, since the script cannot import part of a slug: the reply names the new files still waiting and the exact command that imports them together with the refresh.
3. Run `bash execs/scpts/import.sh` with the user's `--source`/`--slug` passed through unchanged. The script owns the writes: never reimplement it, and never hand-write a star entry — when the script fails, the fix is the script or an honest report, not a forged entry.
4. Report entries added, rewritten (bytes changed), and re-stamped (commit and date only); Step 4 carries the routes.
...
2. Outside `mates/manual/` → copy it in once step 3 is answered, the original untouched, so a file whose source nobody can name never enters `mates/`, where no skill may remove it; a name collision is a question, never an overwrite. Already registered → compare its checksum with the entry's `sha256` first: unchanged, only `source` and `covers` may move; changed, the rewrite is an overwrite asked on its own (conventions §7.7), showing both checksums and what changed where the file is diffable — a fresh copy from its stated source goes on to step 3, and an edit made here is declined (Principle 2, conventions §9e) and stays `tampered` until the file is re-dropped from its source.
3. One question via ask_user_question for what no probe can know: `source` — where this came from (path, URL, or person) — and `covers`, asked on every `register` (conventions §7.7): what the user's request already said is quoted as the proposed answer, and the entry waits for the user to confirm it.
4. Copy a file from outside `mates/manual/` in (step 2), compute `sha256`, write the `##` entry with `source-type: manual` and today's real date. Re-registering an existing file rewrites its entry in place — one entry per file, current state, not history.
...
1. **Coverage.** Files under `mates/` with no entry → `unregistered` — invisible to the writing skills until `register` runs, which fixes only a file under `mates/manual/`. One under a star slug was written outside `execs/scpts/import.sh` (Principle 1): its fix is a re-import with that slug's `--source`/`--slug` when upstream still has it, and otherwise the obsolete-file question (State & File Rules). Entries whose file is gone → `missing` — re-import (star) or re-drop and `register` (manual).
2. **Integrity.** Every file's checksum against its entry's `sha256`. A mismatch is `tampered`: the read-only rule was broken. Show what changed where the file is diffable; the fix is re-import or re-drop and re-register — never keep the edit, and never quietly update the checksum to match, which would launder the tamper into provenance. An entry whose `sha256:` reads `n/a (no sha256 tool on this host)`, or a file this host has no `shasum` or `sha256sum` to hash, is `unchecked` — a degraded check (conventions §3.5), never `tampered` — and the fix is re-import or re-register on a host that has one.
3. **Staleness** — star entries: `bash execs/scpts/import.sh --diff`, which walks every imported slug against its recorded `source:`. `stale` (upstream bytes moved) → the refresh command; `new upstream` → the import command; `missing upstream` → the obsolete-file question (State & File Rules); `tampered` → step 2's finding; `unknown` (that slug's source is not reachable here) → not diffed, said so, with the `STAR_HOME` or path it needs. Manual entries have no upstream to diff — refresh there is the user re-dropping the file — and the report says so instead of pretending to know.
...
- Every question — an unrecognized token or a missing `--source` (Step 0), the import-rewrite confirmation (Step 1), a name collision, a changed checksum, and the provenance question (Step 2), an obsolete file — goes via ask_user_question, one question per call. If it is unavailable (non-interactive `dsh --profile headless`, no human to answer), fall back to plain text and require an explicit answer before the write.
...
- Reply in the language resolved under conventions §7.6; the manifest stays English; paths, hashes, and metric names stay English inside Chinese replies.
