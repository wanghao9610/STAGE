...
2. Outside `mates/manual/` → copy it in, the original untouched; a name collision is a question, never an overwrite. Already registered → compare its checksum with the entry's `sha256` first: unchanged, only `source` and `covers` may move; changed, the rewrite is an overwrite asked on its own (conventions §7.7), showing both checksums and what changed where the file is diffable — a fresh copy from its stated source goes on to step 3, and an edit made here is declined (Principle 2, conventions §9e) and stays `tampered` until the file is re-dropped from its source.
