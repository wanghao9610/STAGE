...
4. Approval: one question via ask_user_question — approve all, approve by group (sections / figures / tables / bibs / styles), or abort. Unapproved rows do not move.
...
- Both confirmation points go through ask_user_question, one question per call. If it is unavailable (non-interactive `dsh --profile headless`, no human to answer), fall back to plain text — still one at a time, still requiring an explicit answer before any write.
