...
7. **立项失败。** 为每个未定义 key、每条 unsupported 或 unverifiable 断言、每处缺失引用、每个卫生缺陷，在 `tasks/cites_followups.md` 的 `## <date>` 标题下追加一条 `- [ ]`——位置、原文、判定、路由：没有笔记 → `stage-refs-curator` 把论文读成一份；播种来的笔记标着 `abstract-and-intro` → 同上，这次正式读一遍；句子写错了 → `stage-sect-drafter`；bib 要修 → `stage-refs-curator`。重跑时把能证明已解决的条目勾掉。
