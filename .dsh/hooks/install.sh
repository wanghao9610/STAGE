#!/usr/bin/env bash
# Register STAGE's DSH hooks in DeepSeek Harness's GLOBAL composition; running it again changes nothing.
#
# Three hooks: model_id provenance, the project-memory index injected from
# .stage/memory/, and the commit guard that declines the git commands the workflow
...
# STAGE project with no per-project editing, and a project without .dsh/hooks.json
# registers nothing.
#
...
for script in stage_model_id.sh stage_memory.sh stage_commit_guard.sh; do
...
if grep -qF "${bridge}" "${patch}" 2>/dev/null; then
  ...
  cp "${patch}" "${patch}.stage-bak"
  {
    printf '\n# --- STAGE hooks (added by .dsh/hooks/install.sh) ---\n'
    printf '# configPath is relative on purpose: it resolves against the directory dsh was\n'
    printf '# launched in, so this one row serves every STAGE project. A project without the\n'
    printf '# file registers nothing. See <project>/.dsh/cordis.patch.yml for the full note.\n'
    printf -- '- insert:\n'
    printf '    - id: stage-hooks\n'
    printf "      name: '%s'\n" "${bridge}"
    printf '      config:\n'
    printf '        configPath: ./.dsh/hooks.json\n'
  } >> "${patch}"
  echo "Registered the STAGE hook bridge in ${patch}"
  echo "  backup written to ${patch}.stage-bak"
  echo "  it now applies in every STAGE project you launch dsh from — no per-project setup."
fi
...
if [ -d "${dsh_home}/profiles" ]; then
  for dir in "${dsh_home}"/profiles/*/; do
    [ -d "${dir}" ] || continue
    name="$(basename "${dir}")"
...
echo
if (( ${#needs[@]} == 0 )) && [ -d "${dsh_home}/profiles" ]; then
