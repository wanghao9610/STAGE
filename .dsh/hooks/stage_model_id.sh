#!/usr/bin/env bash
# STAGE SessionStart hook (DeepSeek Harness) — inject the runtime-reported model
# id into session context so skills record a real model_id instead of "unrecorded".
#
# DSH has no hook table of its own; it runs STAGE's hooks through its Claude Code
