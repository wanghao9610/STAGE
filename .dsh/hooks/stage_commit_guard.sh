#!/usr/bin/env bash
# STAGE PreToolUse hook (DeepSeek Harness) — decline the git commands that break
# the writing workflow conventions §1 in the ways that are expensive to undo. It
# is the floor under INVOLVE=low, which answers the commit offer itself (§1.6,
...
# so the other trees' `Bash` would match nothing here and say nothing. One of
# seven copies — Claude, Codex, Kimi Code and Qwen Code carry the same guard on
# their own PreToolUse, Cursor on beforeShellExecution, Pi on tool_call —
...
deny() { # $1 = one-line reason
    printf '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"%s (declined by .dsh/hooks/stage_commit_guard.sh — hand it to the user to run)"}}\n' "$1"
    exit 0
}
