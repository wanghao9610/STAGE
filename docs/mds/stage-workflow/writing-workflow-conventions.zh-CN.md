...
1. **默认本地执行。** 只委派边界清楚、彼此独立、且委派确实带来实质收益的工作。绝不给每个琐碎的顺序步骤都配一个委派者。**"实质收益"是可判定的**：输入很大、返回很小，且主 agent 事后要重读的是抽查，而不是同一份内容再读一遍。凡本文件或某个 `SKILL.md` 已经要求主 agent 重开同一份证据的地方——每个被审计的数字都要回到它标注的那一行复核（§6.5）、每条评审意见进入逐点台账前都要重读——委派只是把这次读取挪了个地方，并没有省掉它，这种活儿就该留在本地做。**宿主根本不提供委派能力时，本条就是 §6 的全部**：写着"派发"的步骤仍然欠着它那份返回格式，由主 agent 在本地按同样的顺序、照同样的格式把它填出来。**在"有委派"与"没委派"之间还坐着第三种状态，而且它才是常见的那种：宿主带着派发工具，但禁止在用户没要求时使用它。** 许可是一个运行时事实——同一台机器上两个会话可以不一样，仓库里没有任何文件看得见它；因此一份写着"要扇出"的 `SKILL.md` 并不是宿主在等的那个请求，文件替代不了用户开口。所以，一个本该扇出的步骤要问一次，就在它要派发的那一刻问，答案在本次运行剩下的时间里有效；那一刻 skill 本来就要停下来提问的，扇出就搭在那一问里，不另起一问。这是参与度档位罕见地挪不动的一问（§7.7）：未经询问就派发，正是宿主禁止的那件事，所以 `low` 档走本地路径，并在摘要里点名它跳过的那次扇出、以及随之丢掉的那道检查。**可能并行分派的步骤，要在那一步里把自己的阈值写成一个读的人能核对的形式**——规模低于它，就由主 agent 自己读。一个数可以（"超过 6 个文件"），一条针对材料本身的条件也可以（"每个 section 文件一个委派者"、"扫到一轮没有新东西为止"）。"很多章节"两样都不是，本条要挡的就是它。
...
4. **每个委派者都是只读的，而且都不抓取。** 没有任何 STAGE skill 带着"实现型委派者"的派发规范，所以**没有任何委派者改动仓库文件，就是这样**——子代理读交给它的东西（章节、评审、证据文件、论文），把交给它的表格填好返回。远端抓取留在主 agent 手里，理由见第 9 条：委派者想要一条记录，就返回那条查询以及它的答案能结算什么，由主 agent 去跑。第 9 条那个口子随身带回来一处写入，也只有这一处：在它之下抓取的委派者，把每份原始内容缓存到运行目录下它自己的前缀里——没人能重新打开的原始内容不成其为证据，而改成用返回格式把整份文档递回来，等于把扇出的意义整个抵消掉。它写下的是逐字节的原始内容，落在 `wkdrs/` 下——git 忽略它，任何一次运行都能重新生成——一个委派者一个前缀，两个委派者绝不写进同一处（第 2 条）。那不是仓库文件；委派者仍然不写任何仓库文件。
...
9. **抓取不并行分派。** 礼貌速率是对别人服务器许下的承诺，而它由整个会话对该 host 一起花——所以同时跑 N 个抓取者，这个承诺就被破坏了 N 次，而把请求额度拆一拆并不能修好它：额度约束的是总量，礼貌是速率。于是这条规则是结构性的、不是算术性的：**只有主 agent 抓取，一次一个请求，按 host 要求的速率。** 委派者需要一条记录，就返回那条查询以及它的答案能结算什么（第 4 条）；由主 agent 去跑，并套用委派者在知道结果之前就写下的判据——这比委派者自己裁定自己的命中更严，也少了一处需要复核的自报通过（第 3 条）。**这个口子是：委派者可以跑它自己那份职责需要的抓取。** 回应上面那段论证的，是拆的不是额度而是速率：每份任务书里都写明它对某台 host 两次自己的请求之间等多少秒——host 自己要求的间隔乘以并发上限——所以三个委派者面对一台要求每 3 秒一次的 host，各自等 9 秒，合起来正是一个抓取者会产生的速率。上限就是第 2 条那个三，秒数写进任务书，绝不写成"注意礼貌"。随它一起来的有三个条件。每份原始内容都缓存到那个委派者自己的前缀下（第 4 条），而主 agent 在动用委派者从中得出的任何结论之前，先打开那份缓存——原始内容不在磁盘上的结论，什么都没证明。凡是能决定一条发现的抓取，都要带上"什么样的答案能结算它"这条判据，且这条判据在查询跑之前就写进返回里；委派者对自己那次命中的裁定只是参考，由主 agent 亲自拿这条判据去核对缓存下来的记录（第 3 条）。以及，被跟踪的文件所誊写字段的那条记录仍留在这里——那是 §9 意义上的转录，不是一次阅读，也不是扇出该干的事。代价写在授权它的同一处：礼貌从此靠一个没人观察得到的等待，而"判据先于结果定死"从结构性的保证变成了自报。这两样都是有意换出去的，而且都不可能靠问委派者"你照做了吗"再换回来。
