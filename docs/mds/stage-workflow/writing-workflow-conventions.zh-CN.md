...
13. **自由文本就是一句描述，每个 skill 都收一句。** `<skill> [TARGET] [DESCRIPTION] [involve=<level>]` 是整份名册共用的形状：先剥掉 `involve=`（第 7 条），目标按 §5 解析，剩下的就是用户用自己的话说明这次运行是为了什么。它是**一条线索，不是一条命令。** 它可以引出 skill 走哪条路径，也可以提供运行随后写下的文字——"审稿人说消融太薄，把它铺开"既选了路径，又给出了写进意见台账的理由——但它绝不代替任何确认点，绝不替 §5.3 本该发问的目标下结论，绝不让 §9a 本该写成 `\todo` 的数字过关，也绝不授权 STOP 线（§2）上的任何一件事。路径由描述定下的运行，要在写任何东西之前说清自己走的是哪条，这样读错一次只赔一个问题，而不是一处错改。**解析不到任何目标的散文就是纯描述，不是缺目标**：按没给参数运行，并先说明这一点。**孤立一个看着像目标却对不上的词不是描述**：它仍是 §5.3 要问清楚的歧义。若某个 skill 的首参数本身就是自由文本——一篇论文的标题、一张还没有 outline 行的表的用途——那个参数就是描述，别无变化。
