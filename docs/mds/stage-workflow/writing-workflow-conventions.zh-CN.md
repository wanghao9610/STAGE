...
5. **确认点属于主 agent，因为在跟用户说话的是它。** STOP 线上的一切（§2）、每一次提交征询（§1.6）、每一次删除或覆盖前的确认、每一个以"已确认"身份落进 `venue.yml` 的值（§9c）、以及每一处"用户到底指的是什么"的歧义（§5.3）——委派者既不提这些问题，也不回答它们；一份活的下一步是其中之一时，它先回到主 agent 手里，那一步才走。
