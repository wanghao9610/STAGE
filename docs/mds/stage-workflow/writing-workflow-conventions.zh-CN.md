...
4. **每个委派者都是只读的，而且都不抓取。** 没有任何 STAGE skill 带着"实现型委派者"的派发规范，所以**没有任何委派者改动仓库文件，就是这样**——子代理读交给它的东西（章节、评审、证据文件、论文），把交给它的表格填好返回。远端抓取留在主 agent 手里，理由见第 9 条：委派者想要一条记录，就返回那条查询以及它的答案能结算什么，由主 agent 去跑。这也顺带取消了本条原先那唯一一个写入例外——委派者产出的东西没有哪一样必须落盘才可复核，因为原始内容是在本地抓、在本地缓存、在本地读的。
...
9. **抓取不并行分派。** 礼貌速率是对别人服务器许下的承诺，而它由整个会话对该 host 一起花——所以同时跑 N 个抓取者，这个承诺就被破坏了 N 次，而把请求额度拆一拆并不能修好它：额度约束的是总量，礼貌是速率。于是这条规则是结构性的、不是算术性的：**只有主 agent 抓取，一次一个请求，按 host 要求的速率。** 委派者需要一条记录，就返回那条查询以及它的答案能结算什么（第 4 条）；由主 agent 去跑，并套用委派者在知道结果之前就写下的判据——这比委派者自己裁定自己的命中更严，也少了一处需要复核的自报通过（第 3 条）。
