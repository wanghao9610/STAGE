...
STAGE 写作工作流中每个 skill 都遵守的规则。十六个 skill——`stage-proj-adopt`、`stage-evid-curator`、`stage-stry-coach`、`stage-outl-planner`、`stage-sect-drafter`、`stage-tabs-builder`、`stage-figs-designer`、`stage-refs-curator`、`stage-copy-editor`、`stage-clms-auditor`、`stage-cite-auditor`、`stage-peer-reviewer`、`stage-resp-writer`、`stage-subm-packer`、`stage-pstr-builder`、`stage-flow-status`——各有自己的工作流、自己的可写文件范围、自己的质检表。它们共用的部分只在这里存一份。不做分节选读：**每个 skill 每次运行开始时都整份读完本文件。**

**优先级。** 本文件是**基线**。某个 skill 的 `SKILL.md` 可以更**严**——更窄的可写文件范围、更低的阈值、额外的确认点、乃至"本 skill 永不提交"——更严者生效。skill 绝不放松本文件设下的规则。若 `SKILL.md` 里带有下述某条规则的一行摘要，那一行是有约束力的提醒，本文件是完整规则。各 skill 以"规约 §n"的形式引用这些节；编号是冻结的，永不重排。
...
| 主张 claim | 记录表里的一行：论文断言的一句话，连同它写在哪、证据是什么、状态如何 | §8、§9 |
| 记录表 ledger | `notes/claims.md`，把每条主张的陈述处 ⇄ 证据 ⇄ 状态连起来的枢纽 | §8 |
| 可写文件范围 what it may write | 一个 skill 或它派出的子代理准许创建、编辑的路径，此外一律不碰 | §6、§8 |
| 退路 fallback | 首选做法拿不到结果时改走的那条路 | §6、`model_id_spec.zh-CN.md` |
| 并行派发 fan-out | 一次派出多个子代理，各自读写互不重叠 | §6 |
| 大小上限 size limit | 单份工具返回装得下多少；超过约 30 KB 就被存成文件，要再读一次才拿得回来 | 各 `SKILL.md` |
...
| `stage-tabs-builder` | 会话结束时提供一次 | 写出的表格，加上它们的提纲与记录表更新 |
...
2. **`wkdrs/` 永不提交**（§10）。构建产物可重新生成，报告只是某一刻的快照；一次审计真正留下的东西，是它带来的记录表状态翻转和 `tasks/` 条目，而那些都是被跟踪的。一份报告若让人觉得"值得提交"，恰恰说明它本该产出的记录表翻转或 `tasks/` 条目缺失了。`.env` 与机器绑定，被忽略。除此之外，登记表（§8）点名的一切都被跟踪——手稿、证据、笔记、周期目录、`tasks/`。
...
5. **运行开始时就已带有未提交改动的路径，绝不暂存。** 提问时点名这些路径——`low` 级别下则在最终回复里点名——让用户先自行提交或 stash；一个 skill 的提交绝不能捆进它没做过的工作。
6. **绝不无声无息地提交。** 提交与否是一道判断题（§7.7），而它推荐的答案是"提交"：本地提交不对外发布任何东西，时机不对的一次提交代价也不过是一句 `git reset --soft`。`medium` 与 `high` 把它作为独立问题提出（§7.2），拒绝始终合法；`low` 不问直接提交。两种情形下最终回复都要点名每一次提交，连同它的 message 和文件数——会问的级别保留否决权，不问的级别保留事后复核。`low` 级别下没人过目暂存清单，规则 5 就独自扛起全部分量：运行开始时先给 `git status` 拍个快照，绝不暂存那时已经脏了的路径。

**守卫。** 每棵工具树都配一个 `stage_commit_guard.sh` 钩子，专门拒掉那些"违反上述规则且事后难以收拾"的命令：一把梭或强制暂存（`add -A`、`add .`、`add -u`、`add -f`、`commit -a`），规则 3 点名的历史改写（`commit --amend`、`rebase`、`reset --hard`、`filter-branch`），一次按键就能丢掉工作的强制分支操作（`branch -D`/`-f`、`switch -C`/`-f`、`checkout -B`/`-f`），任何删除或移动 tag 的动作（规则 4——冻结 tag 正是 `SUBMISSION_<date>.md` 所指向的东西），以及暂存文件超过 10 MB 的提交，因为把一个构建 PDF 或图像大文件从历史里清出去，需要的恰恰是那些被禁的改写。`push` 是有意不在其列的：这里没有任何规则会让 skill 更倾向于 push，而用户若直接要求一次 push，就该拿到它。守卫是散文规则底下的一层地板，绝不是它的替代品——它一次只读一行 shell，无法解析引号，读不确定的东西一律沉默放行。它拒掉的命令，是用户自己去跑的。
...
5. **绝不顺手给章节重新编号。** `<n>_` 前缀是承重的：提纲行、记录表的 `Stated in` 列、`main.tex` 里的 `\input` 顺序都建在它上面。重新编号是 `stage-outl-planner` 的一次刻意操作，把文件、提纲、`main.tex` 与记录表一起改掉——绝不是起草时的副作用。
6. **文件与提纲必须一致。** 有 `manus/secs/` 文件却没有提纲行，或某行指向的文件不存在，都是要报出的漂移（`stage-flow-status` 会点名），而不是在任务中途默默修掉的东西。

## 6. 委派

1. **工作能切开的地方就并行分派。** 一次运行的各个部分彼此独立——读的是不同文件，或写的是不同文件，谁也不必等别人的结果才能开工——那么委派就是它默认的形态：一份一个委派者，放在同一条消息里派出去，让它们并发地跑。**由主 agent 自己决定，而且它一个人决定**：仓库里没有任何文件会先去问用户要许可，因为扇出改变的是把活干完的方式，不是这份活本身；一次运行停下来问"我可不可以快一点"，问掉的正是它想省下的时间。留在同一个上下文里的，是切开就会坏掉的那些：互相喂结果的步骤，以及需要整幅画面同时摆在眼前的判断。**宿主根本不提供委派能力时，本条就是 §6 的全部**——写着"派发"的步骤仍然欠着它那份返回格式，由主 agent 在本地按同样的顺序、照同样的格式把它填出来，并在摘要里说明本次是在本地跑完的。宿主带着派发工具却拒绝这次调用，答案一样：退回本地、把活干完，并在摘要里点名哪一次扇出没能发出去。**会扇出的步骤，要在那一步里把自己的阈值写成一个读的人能核对的形式**——规模低于它，主 agent 就自己做。一个数可以（"超过 6 个文件"），一条针对材料本身的条件也可以（"每个 section 文件一个委派者"、"扫到一轮没有新东西为止"）。"很多章节"两样都不是，本条要挡的就是它。
2. **交给委派者的是**：确切的文件清单、逐项列明字段并以"除此之外什么都不返回"收尾的返回格式、以及逐字写明的范围（"只做这些条目"）。**并发的委派者之间文件归属互不重叠。** 这一条不放宽，而且它不是审计规则：两个 agent 改同一个文件，会把彼此的修改互相抹掉，而被覆盖掉的那次写入，下游没有任何环节能把它找回来——所以在扇出持续的这段时间里，会写入的委派者独占它自己那几个文件，把一个文件同时交给两个委派者是缺陷，不是排得更紧。**同时跑几个由主 agent 定**，按活的规模来定，而不是照着一个固定上限，并且要把这个数写下来，因为第 9 条的等待时间要拿它来除。
3. **主 agent 拥有集成权。** 它把各份返回拼起来，裁定两份返回互相打架的地方，并亲自跑那几道确定性关卡——`execs/run.sh` 和 `execs/scpts/lint.sh`——因为它们是机械的、便宜的，而且能抓住任何自报都抓不住的东西：一个解析不出来的引用、`\todo` 的条数、超出预算的页数。**它不重读委派者已经读过的东西。** 换来扇出的正是这一点，代价写在这里而不是留给日后发现：委派者对自己覆盖面的那份自述——读了多少节、核了多少条目——就是本次运行手里的全部，所以数目低于交给它的数量意味着差额要重新派发，而不是结果变小了；返回和关卡打架，输的是返回。
4. **委派者可以写入，但只在它的范围内，并且受所有约束写入者的规则约束。** 委派转移的是工作，绝不降低这份工作可以产出什么的标准；每条规则跟着写的人走，而不是留在主 agent 身上。§9 的虚构边界约束的是写下那句话的委派者：进入 `manus/` 的数字，要带上那个委派者本次运行亲自打开的那条 `mates/` 记录的 `% src:` 锚点，否则就是一个 `\todo{}`——委派者同样没有第三种状态。§8 的格式约束它写下的每一行。`mates/` 对所有人只读（§10）。有两样东西留在主 agent 手里，理由与信任无关：**git**，因为并发的 `git add` 和 `git commit` 会抢同一个索引，所以本次运行那唯一一次提交（§1）是主 agent 的；以及**对话本身**，见第 5 条。
5. **确认点属于主 agent，因为在跟用户说话的是它。** STOP 线上的一切（§2）、档位要求问出提交时的那次征询（§1.6）、每一次删除或覆盖前的确认、每一个以"已确认"身份落进 `venue.yml` 的值（§9c）、以及每一处"用户到底指的是什么"的歧义（§5.3）——委派者既不提这些问题，也不回答它们；一份活的下一步是其中之一时，它先回到主 agent 手里，那一步才走。
6. **换个视角复核的委派者**——被派去重读主 agent 自己写出的散文或自己做完的审计——只要被复核的产物会把关下游的工作，就值得派：盲区是结构性的，而"再读一遍"恰恰是一句话的作者没法对自己那句话提供的东西。它是主 agent 事后仍要自己再读一遍、而不是拿它代替自己读的那一类委派，因为换来的是第二双眼睛，不是省下一次阅读。这套工作流把它制度化的形态就是 `stage-peer-reviewer`——一个 skill，而不是临时起意的委派。
7. **没有任何 skill 扣着委派不给。** 没有 `SKILL.md` 禁用派发工具，也不允许有：一个没什么可扇出的 skill，靠"不写出任何扇出点"来表达这件事，而不是靠拒绝扇出。凡是某个 skill 的正文把某一步留在本地的——用户正坐在里面的访谈、只有一次脚本调用的关卡、还没等委派者返回就已经读完的读取——那就是第 1 条要的那个阈值，只不过它取的方向恰好是"这里不派"，并且写明为什么。
8. **参与度档位同样管到委派**（§7.7）。`high` 档，并行派发要在动手前连同它的切分方式一起说明；`medium` 和 `low` 档则不声张地跑。任何档位下，决策记录（§7.8）都要写明本次运行做了并行派发、以及是怎么切分的——切分和别的裁量题一样，是一次判断。
9. **抓取要把速率除开。** 礼貌速率是对别人服务器许下的承诺，而它由整个会话对该 host 一起花——所以同时跑 N 个抓取者，这个承诺就会被破坏 N 次，而把请求额度拆一拆并不能修好它：额度约束的是总量，礼貌是速率。修法是算术的，而且它写在任务书里：**每个会抓取的委派者都被告知，它对某台 host 两次自己的请求之间要等多少秒——host 自己要求的间隔，乘以这次扇出派出了多少个委派者**（第 2 条那个数）。三个读者面对一台要求每 3 秒一次的 host，各自等 9 秒，合起来正是一个抓取者会产生的速率。秒数是写进去的；"注意礼貌"不是一个速率。每份原始内容在被读之前，都先缓存到运行目录下那个委派者自己的前缀里——没人能重新打开的原始内容什么都没证明，而改成用返回格式把整份文档递回来，等于把扇出的意义整个抵消掉——一个委派者一个前缀，两个委派者绝不写进同一处（第 2 条）。代价写在授权它的同一处：礼貌从此靠一个没人观察得到的等待。
...
5. **结论先行**，然后是证据，最后是转交给下一步的建议——而对那十个 agent 可以自己启动的 skill 来说，"转交"就是直接把它跑起来，而不是打印一条等人来敲的命令（§11.4）。下一步是打印而非直接执行的情形——slash-only 的 skill、目标还没定、STOP 线上的任何事、要交回用户去跑的命令（§2）——回复的收尾一行就写出**那条精确的命令**，用本 harness 自己的调用语法逐字给出，绝不改述；一次没有什么可转交的运行，也在这一行明说，而不是把这一行省掉。读者永远不该问"我现在敲哪条命令？"。
...
13. **自由文本就是一句描述，每个 skill 都收一句。** `<skill> [TARGET] [DESCRIPTION] [involve=<level>]` 是整份名册共用的形状：先剥掉 `involve=`（第 7 条），目标按 §5 解析，剩下的就是用户用自己的话说明这次运行是为了什么。它是**一条线索，不是一条命令。** 它可以引出 skill 走哪条路径，也可以提供运行随后写下的文字——"审稿人说消融太薄，把它铺开"既选了路径，又给出了写进意见记录表的理由——但它绝不代替任何确认点，绝不替 §5.3 本该发问的目标下结论，绝不让 §9a 本该写成 `\todo` 的数字过关，也绝不授权 STOP 线（§2）上的任何一件事。路径由描述定下的运行，要在写任何东西之前说清自己走的是哪条，这样读错一次只赔一个问题，而不是一处错改。**解析不到任何目标的散文就是纯描述，不是缺目标**：按没给参数运行，并先说明这一点。**孤立一个看着像目标却对不上的词不是描述**：它仍是 §5.3 要问清楚的歧义。若某个 skill 的首参数本身就是自由文本——一篇论文的标题、一张还没有 outline 行的表的用途——那个参数就是描述，别无变化。
...
| 证据 | `execs/scpts/import.sh` + `stage-evid-curator` | `mates/<slug>/**`、`mates/manual/**`，记录表 `mates/MANIFEST.md` | 每条：`source-type:`、`source-stamp:`、`imported:` |
...
| 主张记录表 | `stage-stry-coach` 创建；`stage-sect-drafter`、`stage-tabs-builder`、`stage-clms-auditor`、`stage-resp-writer` 更新 | `notes/claims.md` | 每条主张的 `Status` 列 |
...
**每份工作流产物都记下写它的模型。** 每个生产者把 `model_id` 写进它在 `notes/`、`tasks/`、`cycls/`、`wkdrs/reports/` 下创建的文件的 frontmatter；还没有 frontmatter 块的文件——`notes/refs/refs_index.md`——为它加一个。取值是运行时为本次写入会话报出的模型 id，原样抄录；而运行时确实会报：STAGE 的 `stage_model_id.sh` 钩子把它写进会话上下文，Claude Code 还会在系统提示里写明。那行注入缺失，或带来的是一条恢复命令而不是 id 时，`model_id_spec.md` 给出各运行时的退路——先跑它，再考虑写 `unrecorded`，那是留给"会话里任何地方都没写明模型"的取值。绝不凭行为推断，绝不去想"这大概是哪个模型"，也绝不把一份产物的取值抄到另一份上。

**而 `model_trail` 记的是跨写入者的流水。** `model_id` 只说明一次写入；这些文件多半是跨很多次会话写成的——一本被五个 skill 改到论文完稿的记录表、每个周期重排一次的提纲、一篇一篇长起来的参考文献库——单个字段在那里只描述最后一次。所以凡带 `model_id` 的产物，旁边都带一份只追加的 `model_trail`：每次写入会话一条，`{ date, model, skill, scope }`，其中 `scope` 用这份文件自己的词汇说清那次会话写了什么——claim ID、章节号、评审要点、表格行。只追加，绝不改写已有条目，并让 `model_id` 始终镜像最后一条，好让一次普通的 grep 仍然有效。一次写成的产物——每份带日期的报告、模拟评审、回复、投稿记录——只有一条；重新生成的产物另起一条新流水，而不是接着它所替换的那一代往下写。形状见 §8.1，这一对字段进入 §8.4–§8.11 每个 schema 的 frontmatter，不在每处重复列出。
...
`stage-flow-status` 是唯一把它们汇到一起的读者——按产物报出最后写它的模型、它的流水有多长，并点名每一份缺流水的登记产物，那正是这些字段存在之前写成的文件的样子。没有任何东西由它们生成，所以也没有一份需要保持同步的记录表文件。
...
**优先级，也正是这份文件做成 schema 而不是一段品味自述的理由。** §9 压过它，其次是 `notes/notation.md`，再次是 venue 的格式规则，最后才是这份档案。具体地说：没有任何档位能授权一个数字、一个引用键、或者拿掉一个 `\todo{}`（§9a）；没有任何档位能盖过规范术语或某个缩写的首次展开（§8.6）；并且**没有任何档位能改变一句话断言了什么。** `hedging: minimal` 收紧的是措辞，绝不撤掉证据所要求的限定语——主张的强度住在记录表行里（§8.1），不住在偏好里，而一份非得压过上面某一条才成立的档案，本身就是错的。那一刻运行要把这件事说出来，并且不动那句话。
...
- **"追溯"指的是整条链。** 在表格里：单元格 → 它所在行的 `% src: mates/<...>#<anchor>` 注释 → 一条带指纹的 MANIFEST 条目 → 该取值确实出现在那个证据文件里。在正文里：句子 → 陈述它的那条记录表行（`Stated in` 点名本节）→ 该行的 `Evidence` 链接 → 指纹。`stage-clms-auditor` 为每个数字走完这两条链，并逐个判定 **matched / mismatched / unsourced**。
...
- **执行：** `lint.sh` 统计 LaTeX 真会排版出来的那些 `\todo{`——每个候选行在判定前会先从第一个未转义的 `%` 起剥掉注释，所以一个被注释掉的标记、或一条只是提到这个宏的注释，都不算失败——非零即硬失败，`stage-subm-packer` 拒绝在其之上打包；`stage-clms-auditor` 追溯每个数字，并为每次失败开一条 `tasks/` 条目；每个"知道但没指纹"的取值都带着一条 `unsourced` 记录表行，让这笔债有名字。
...
- **有两个 skill 可以越过手稿已经引用的范围去看**，而且都在同一份引用完整性约定之下：一个工作被点名，要么是在**白名单**里（已在 `manus/bibs/reference.bib` 中），要么是**已核实**——在那次运行中抓取过，且记录（标题、作者、年份、venue、URL）与找到它的查询都被记录下来，返回为空的查询也一并记录。凭记忆想起的工作一律按不存在处理；抓不到的东西写成一个方向（"查一查 X 方向是否已有前作"），绝不写成一个具名事实。`stage-peer-reviewer` 检索是为了攻击这篇论文的定位，并把找到的工作写进模拟评审。`stage-refs-curator discover` 检索是为了给参考文献底盘提议候选，而且只提议：由作者来挑，检索翻出来的东西在被挑中之前一律不进 `reference.bib`、也不进任何阅读笔记——一个按自己口味挑定参考文献的代理，攒出来的定位到评审时没人替它辩护得了。这是本工作流仅有的两处被批准的实时检索用途。每个 skill 用结构给自己的检索定界，而不是用额度——一轮扫到不再翻出新东西就结束、一次扩展绝不许扩大自己的输入、一组在第一条查询之前就定死的问题——因为凭空定出来的额度是一个谁也辩护不了、谁也没法拿日志去核的数字，而一个停止条件本来就写在日志里。礼貌速率仍然是一个数：它是对别人服务器许下的承诺，不是我们自己的参数。§2 的批量抓取红线依然生效；周期标了 `anonymized: true` 或 `.env` 设了 `ANON=true` 时，两者都退到只用主题词——一条带着论文标题或它某个句子的查询，会让握有检索日志的人认出一篇匿名投稿。
...
- 为什么是绝对的：指纹链之所以是证据，只因为它底下的字节不会漂移。一旦容忍了一次手工编辑，记录表里的每一个 `verified` 就都变成了"针对某个可能被谁调过的东西验证过"。
...
- 这类要求一定会来，而且听起来都很有道理："三小时后截稿，先把 54.6 填进去，回头再补来源"；"数字就在我这条消息里，你看得见"；"ImageNet 多大你总该知道吧"。拒绝从来不是干巴巴的一句不行——skill 拒掉这条捷径，同时点明能达到同样效果的诚实路径：
  - `\todo{}` 的写法加一条 `unsourced` 记录表行——三十秒，能编译，并且以一笔*有名字*的债而不是一个藏起来的洞熬过审计；
...
| 导入的证据（只读） | `mates/<source-slug>/**`，镜像上游路径；手工登记的投放在 `mates/manual/**`；记录表 `mates/MANIFEST.md` |
...
4. **稿件永远编译成预印本；venue 的版式是一份生成出来的副本。** `manus/stys/` 放三个文件，换 venue 时哪个被替换是承重的：
   ...
   `venue.yml` 并列——绝不放进 `manus/`：那是 `lint.sh` 扫描的目录树，模板包自带的示例 `.tex` 会污染它的 `\todo`
...
| `stage-refs-curator` | 参考文献、阅读笔记、主题发现、定位 |
...
| `stage-resp-writer` † | 评审意见 → 要点记录表 → 回复 + 承诺 |
