...
4. **每个委派者都是只读的，而且都不抓取。** 没有任何 STAGE skill 带着"实现型委派者"的派发规范，所以**没有任何委派者改动仓库文件，就是这样**——子代理读交给它的东西（章节、评审、证据文件、论文），把交给它的表格填好返回。远端抓取留在主 agent 手里，理由见第 9 条：委派者想要一条记录，就返回那条查询以及它的答案能结算什么，由主 agent 去跑。第 9 条那个唯一的口子随身带回来一处写入，也只有这一处：被派去读某份文档的委派者自己抓了那份文档，就把原始内容缓存到运行目录下它自己的前缀里——没人能重新打开的原始内容不成其为证据，而改成用返回格式把整份文档递回来，等于把扇出的意义整个抵消掉。它写下的是逐字节的原始内容，落在 `wkdrs/` 下——git 忽略它，任何一次运行都能重新生成——一个委派者一个前缀，两个委派者绝不写进同一处（第 2 条）。那不是仓库文件；委派者仍然不写任何仓库文件。
...
9. **抓取不并行分派。** 礼貌速率是对别人服务器许下的承诺，而它由整个会话对该 host 一起花——所以同时跑 N 个抓取者，这个承诺就被破坏了 N 次，而把请求额度拆一拆并不能修好它：额度约束的是总量，礼貌是速率。于是这条规则是结构性的、不是算术性的：**只有主 agent 抓取，一次一个请求，按 host 要求的速率。** 委派者需要一条记录，就返回那条查询以及它的答案能结算什么（第 4 条）；由主 agent 去跑，并套用委派者在知道结果之前就写下的判据——这比委派者自己裁定自己的命中更严，也少了一处需要复核的自报通过（第 3 条）。**唯一的口子是委派者被派去读的那份文档。** 一次给每个委派者派一份文档的扇出，可以让它们各自去抓自己那一份，仅此而已。回应上面那段论证的，是拆的不是额度而是速率：每份任务书里都写明它对某台 host 两次自己的请求之间等多少秒——host 自己要求的间隔乘以并发上限——所以三个委派者面对一台要求每 3 秒一次的 host，各自等 9 秒，合起来正是一个抓取者会产生的速率。上限就是第 2 条那个三，秒数写在任务书里，绝不写成"注意礼貌"；此外一切仍留在这里：找到那份文档的检索、bib 条目或引用所誊写的每一条记录，以及每一次指标调用。代价是真的，也正是这个口子的边界所在——礼貌从此靠委派者遵守一个没人观察得到的等待，第 3 条对它没有抓手，所以它只到那个委派者被派去读的那一页为止，再远就不行。
