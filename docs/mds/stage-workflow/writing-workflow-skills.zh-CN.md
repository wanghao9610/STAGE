...
**每次调用起草或修订一节**，按规约 §5 解析。它装载该节的简介、映射到该节主张的证据、相关的记录表行、`notes/notation.md`、可选的作者文风档案与自然写作指南，然后写 tex。段落先给出具体主张，技术术语保持稳定；段落级检查清理集中出现的公式化支架，同时不改变受保护内容。任何缺 `mates/` 指纹的数字都写成 `\todo{...}`——没有第三种状态（规约 §9a）。收尾时它更新记录表（`Stated in`，状态 → `drafted`）、把新符号追加进记号表、并更新该节的提纲行，于是起草一节就把整套账目一起带动了。
...
其中的清晰度视角还会使用自然写作指南：宣传式过度宣称、模糊归因、术语轮换、各节重复模板、泛化结论和聊天机器人残留都作为带锚点的呈现问题报告，绝不被说成 AI 作者身份的证明。
