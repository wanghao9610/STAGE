...
打磨那一遍，作用于一节或整篇手稿：清晰度、流畅度、自然的学术表达、对着 `notes/notation.md` 的一致性（术语规范、缩写首次使用），以及朝提纲预算裁短篇幅。编辑前先冻结事实、数字、key、来源锚点、todo、归属、主张强度和证据要求的限定语；随后判断模式组合及其修辞作用，在段落尺度重写，而不是照禁词表换词。编辑后逐项对照受保护清单；无法证明含义未变的地方只报告、不施行。最后运行构建和 lint，散文警告仍只供复核。它无法就地修掉的系统性问题写进 `wkdrs/reports/POLISH_<date>.md`，好让这些问题被刻意处理，而不是被顺手抹平。
