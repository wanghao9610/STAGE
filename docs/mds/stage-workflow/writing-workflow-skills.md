...
One command stands outside the roster: `stage-auto <goal> [involve=<level>]` pursues a stated goal, such as `stage-auto the method section drafted and its numbers audited`. It runs `stage-flow-status` first, then takes the next action each run names, starting the ten unmarked skills itself, one unit of work per start, because typing the command is your decision, made once for the pursuit; an action that does not advance the goal is listed rather than taken. At a † skill it stops and prints the exact command, and it stops the same way at a STOP-line action, a question only you can answer, a venue value you have not confirmed, work only you can unblock (evidence to import, a promise box to tick), a repeat of the same start, or a pass that changed nothing. Confirmation points are still asked, each started skill keeps its own commit step, and nothing in a goal run imports evidence or enters a venue fact (conventions §11.5).
