...
Five of them are read by the entrypoint scripts. `STAGE_LANG` and `INVOLVE` are the exceptions: no entrypoint reads either, the skills do (§7.6, §7.7) — and `INVOLVE` is read once more by the harness hooks that answer a permission prompt at `low` (§7.7).
