...
| fingerprint | the `mates/MANIFEST.md` entry pinning an evidence file: source-type, source, commit, source-stamp, sha256, import date | §8 |
...
| fan-out | several subagents, run concurrently or in turn as the main agent judges, on independent pieces, no two of them writing the same file | §6 |
...
| staleness | an imported file whose upstream stamp no longer matches its recorded `source-stamp:`; detected by exact comparison, never mtime | §8 |

## 1. Git

**The discipline is one commit per skill run** — never unannounced, staging only that run's work, with the subject prefix naming the skill: `stage-sect-drafter: 3_method — first full draft`, `stage-evid-curator: import xseg results`. One skill, one prefix, so the log separates by skill.
...
| `stage-cite-auditor` | offered once after the audit, when it filed or closed follow-ups | `tasks/cites_followups.md` — findings stay in the `wkdrs/` report, and the bib is read-only here: a bib repair routes to `stage-refs-curator` |
...
| `stage-resp-writer` | offered once when the run ends | the `RESPONSE_*` file, `tasks/<cycle>_promises.md`, ledger downgrades |
| `stage-subm-packer` | asked once at pack time, at every involve level (§7.7); one more per `convert` run, for what it wrote outside `wkdrs/` | `cycls/<cycle>/SUBMISSION_<date>.md`; the `freeze/<cycle>_<date>` tag then lands on that commit — the package itself stays in `wkdrs/builds/`. The `convert` commit is separate and stages only what that run wrote outside `wkdrs/` — the kit under `cycls/<cycle>/template/` and `tasks/<cycle>_venue.md` — so the freeze commit stays the one file it claims to be |
| `stage-pstr-builder` | offered once when the run ends | `cycls/<cycle>/poster/POSTER_PLAN.md`, `poster.tex`, and, when a kit was supplied, the registered kit under `cycls/<cycle>/poster/template/` — the render stays in `wkdrs/builds/poster/` |
...
5. **A path that already carried uncommitted changes when the run started is never staged.** Name those paths when asking — or, at `low`, in the final reply — so the user can commit or stash them first; a skill's commit must never bundle work it did not do. A path already staged when the run started stays out of the commit too: a skill commits with a pathspec, `git commit -m <msg> -- <paths this run staged>`, since a bare `git commit` takes the whole index — an adoption's `git mv` renames included.
...
**The guard.** Every tool tree ships a `stage_commit_guard.sh` hook that declines the commands breaking these rules where the break is expensive to undo: blanket or forced staging (`add -A`, `add .`, `add -u`, `add -f`, `commit -a`, `commit .`), the history rewrites rule 3 names (`commit --amend`, `rebase`, `reset --hard`, `filter-branch`, `filter-repo`), the forced branch operations that discard work in one keystroke (`branch -D`/`-f`, `switch -C`/`-f`, `checkout -B`/`-f`) and the whole-tree discards that lose the same work (`checkout .`, `restore .`, `stash drop`/`clear`, and `clean -f`, which also deletes `mates/` imports still waiting for the user's commit), any deletion or move of a tag, local or on the remote, where the guard can tell it is one — spelled `refs/tags/…`, shaped `freeze/…`, or a tag in this clone (rule 4 — a freeze tag is what `SUBMISSION_<date>.md` points at), and any add or commit that would stage a file over 10 MB, since clearing a build PDF or a figure blob back out of history needs exactly those rewrites. Otherwise `push` is deliberately absent: nothing here makes a skill likelier to push, and a user who asks for one directly should get it — only a push that deletes or moves a tag (`--delete`, a `:<tag>` refspec, a forced push of a tag or of `--tags`, a pruned or forced wildcard over `refs/tags/` or all of `refs/`, `--mirror`, each also in git's abbreviated spelling) is declined. The guard is a last check behind the prose, never a replacement for it — it reads one shell line at a time and cannot resolve quoting, so a declined command quoted in a message after a `;`, `&`, `|`, or parenthesis is declined with the line (a message written through a heredoc, `git commit -F- <<'EOF'`, is read as data), and silence is its answer to anything else it cannot read confidently. What it declines is the user's to run.

**Why it matters.** `SUBMISSION_<date>.md` claims "this tag is what went to the portal", and that is only true because commits come from named skill runs and tags from exactly one skill.
...
- **Deleting evidence** under `mates/` — its writers only add or replace whole files (§9d), and removal is the user's git-visible act — deleting a freeze tag, any git history surgery (§1 bars the last two outright); confirmation does not unlock any of them. Replacing a whole evidence file through `execs/scpts/import.sh` or a re-registration is not on this line: it is a mandatory confirmation point (§7.7). Nor is an in-place edit, which §9d and §9e forbid at every level.
...
Five of them are read by the scripts under `execs/`. The other five no script reads; the skills do — `STAGE_LANG` and `INVOLVE` (§7.6, §7.7) and the three tier keys (§11.6) — and `INVOLVE` is read once more by the harness hooks that answer a permission prompt at `low` (§7.7).
...
5. **Lead with the outcome**, then the evidence, then the routing to the next step — printed, not taken, unless the user's own request already asked for that successor (§11.4): a run ends with its report and starts nothing on its own. The reply's closing line carries that next step as the **exact command**, verbatim in this harness's own invocation syntax — the prefix §11 gives each harness, `/stage-<name>` in Claude Code — never a paraphrase, whether it names one of the ten, a slash-only skill, or a command handed to the user under the STOP line (§2); and a run with nothing left to route says so in that line rather than dropping it. Where a skill names several routes, the closing line carries the first whose precondition this run left true, with its target, and the others stay as report lines above it; with no target settled, it is `stage-flow-status`, which picks one. A run that stops at a question it asked closes on that question: the answer is what comes next, and a route that needs the answer stays a report line above it. A printed command recommended to run at an `involve` level other than the one `INVOLVE` in `.env` resolves to (item 7) carries the token spelled out — `/stage-sect-drafter 3_method involve=low` in Claude Code — so the line works pasted as printed; at the level `.env` already gives, the bare command is the whole recommendation. The recommended level is the one this run resolved, unless the user asked for another. The reader should never have to ask "what do I run now?".
6. **`STAGE_LANG` sets the language of replies and of what a run writes.** `.env` `STAGE_LANG=en|zh` (§3) fixes both: chat replies, and the Markdown a run newly writes — `notes/`, `tasks/`, `wkdrs/` reports. Unset, empty, or any other value → follow the user's dialogue language, so a Chinese conversation gets Chinese replies; an explicit in-conversation request beats both and stands for the rest of the run. A skill **resolves it once at the start of the run**, reading the needed `.env` controls (`STAGE_LANG`, `INVOLVE`, and the tier keys of §11.6) together and reusing them within the run until they change. It governs what a run **writes**, never a retranslation of what is already on disk: an existing document keeps the language it was written in, and changing one is an explicit user request, not a side effect of flipping the variable.

   **What stays English whatever `STAGE_LANG` says.** Two things leave this repository to be read by people who did not write them, and both are always English: **everything under `manus/`** — prose, captions, `% src:` comments, the text inside `\todo{}` — and **the response to reviewers** (`cycls/<cycle>/response/`), which a program committee reads. The manuscript's language belongs to the venue and the user; no dialogue language and no `STAGE_LANG` value ever rewrites it. Alongside those, **every structural literal stays English inside a document written in any language**: frontmatter keys and their values, ledger statuses (`proposed` / `drafted` / `verified` / `unsourced` / `weakened` / `dropped`), claim and review-point IDs, file paths, bibkeys and everything from `reference.bib`, venue names, dataset and metric names, and every string a script greps. A Chinese note with English structure stays machine-readable; a translated status value breaks `lint.sh` and every skill that reads the row. `stage-peer-reviewer`'s review artifacts — the `SIM_REVIEW_<date>.md` or `REFEREE_<date>.md` and its run directory — are English as well, because a simulated review must read like a received one; a `中文要点摘要` closes the meta-review when the resolved language is Chinese.
...
7. **The `involve` level: the user chooses how much is asked.** It moves the questions still open, never the user's authority: it revokes no approval the user already gave and grants none the user did not (item 2). Every question a workflow poses is one of three kinds. **Mandatory confirmation points** are asked at every level, and only their own answer settles one — not the level, not a description (item 13), not an approval of the surrounding work: anything on the STOP line (§2 — submissions above all), every confirmation before a deletion or an overwrite, every `venue.yml` value entering as confirmed (§9c), the provenance question before a file is registered as evidence (§9e), the promise-keeping question before a box in `tasks/<cycle>_promises.md` is ticked, each confirmation point where one of the six slash-only skills settles the decision it exists for (§11.1), and every ambiguity about what the user meant (§5.3 is the section-name case). **Judgment calls** — questions item 3 equips with a marked recommendation, where every offered option is safe — are what the level moves, and an explicit approval of the same operation and scope answers one in advance (item 2). **Derivable details** — anything with a conventional default — are decided silently at every level; they were never questions.
   ...
   **The token is not an argument.** `involve=<level>` is stripped from the invocation before anything else is resolved — the section (§5.2), the cycle, the mode. This holds in **every** skill, whatever its `SKILL.md` says about the level: a skill matching its first argument against outline titles must not see `involve=low` and treat it as a section name. A skill that accepts no arguments at all still strips it.
   ...
   - `low` — a judgment call is not asked: take the option you would have marked recommended, and log it (item 8). The commit offer is one of them (§1.6), and what it recommends is to commit; the reply still names what was committed. Taking a recommendation is never permission to widen the run's scope (§11.3) or to start the next run the reply names (§11.4). In Claude Code, Codex, and Qwen Code the level also reaches the harness: at `low`, `stage_involve_gate.sh` answers the permission prompt before a file edit inside the project, while `mates/`, a venue kit under `cycls/<cycle>/template/`, and the dot-directories at its root keep theirs. In Claude Code, `stage_bash_gate.sh` answers the one before a shell command too, except a command that deletes, overwrites a tracked file, writes under `mates/` other than through `execs/scpts/import.sh`, does more than read a venue kit or `.env`, installs, pushes or otherwise sends outward, launches a job, pulls, rewrites or prunes history, switches a branch, or creates, moves, or deletes a tag — those keep their prompt at every level. Codex and Qwen Code take that level from `.env` alone; Claude Code's gates also read the `involve=` token of the session's most recent STAGE command, which holds until the next one, past the end of the run, and plain language mid-run reaches no hook, except in a `stage-auto` goal run, whose next start carries the changed level as its `involve=` token. A genuinely open question has no recommendation to take, so it is asked at every level — and when unsure which kind a question is, treat it as the more interactive kind.
...
| Response | `stage-resp-writer` | `cycls/<cycle>/response/RESPONSE_<date>.md`, promises in `tasks/<cycle>_promises.md` | promise checkboxes, ticked by the skill whose revision keeps each |
...
**Every workflow artifact records the model that wrote it.** Each producer writes `model_id` into the frontmatter of what it creates under `notes/`, `tasks/`, `cycls/`, and `wkdrs/reports/`; a file that has no frontmatter block yet — `notes/refs/refs_index.md` — gains one for it. The value is the model id the runtime reports for the writing session, copied verbatim, and the runtime does report it: STAGE's `stage_model_id.sh` hook states it in the session context, and Claude Code names it in the system prompt besides. A file a delegate writes records the delegate's id, never the session's (§11.6): the delegate runs the command its own start line injected, against its own record, and returns the id it wrote. Where that line is missing, or carries a recovery command in place of an id, §13 (harness hooks and model provenance) holds the per-runtime fallback — run it before writing `unrecorded`, which is the value for a session that names no model anywhere. Never infer it from behavior, never reason about which model this is "probably", and never copy one artifact's value into another.

**And `model_trail` records the flow across writers.** `model_id` names one write; most of these files are written across many runs — a ledger five skills edit for the life of the paper, an outline replanned each cycle, a reference base grown paper by paper — and there a single field describes only the last one. So every artifact carrying `model_id` under those four directories carries an append-only `model_trail` beside it (a memory, §12, carries `model_id` alone): one entry per run that writes it, `{ date, model, skill, scope }`, where `scope` names what that run wrote in the file's own vocabulary — claim IDs, section numbers, review points, table rows. Append, never rewrite a past entry, and keep `model_id` mirroring the last entry so a plain grep still works. A write-once artifact — every dated report, simulated review, response, and submission record — has exactly one entry; a regenerated one starts a fresh trail rather than extending the trail of the generation it replaced. The shape is the one in §8.1, and the pair joins the frontmatter of every schema in §8.4–§8.12 without being repeated in each.
...
`stage-flow-status` is the only reader that puts them together — for each `notes/` file and `notes/refs/refs_index.md` its scan reads, the model that last wrote it and how long its trail is, plus those of them whose trail is missing, which is what a file written before these fields existed looks like. Nothing is generated from them, so there is no ledger file to keep in step.
...
model_trail:                    # append-only: one entry per run that writes it, never rewritten
  - { date: YYYY-MM-DD, model: <id | unrecorded>, skill: stage-…, scope: <what this run wrote> }
---
# Claim ledger
...
- source-commit: <sha | <sha>-dirty | n/a>
...
`star` entries are managed by `import.sh`; `manual` entries are written by `stage-evid-curator`. Both write all seven fields.
...
references_in_limit: false    # lint and the venue copy count content pages only
...
confirmed:                    # date the USER confirmed this file as echoed, blanks included
                              # (a blank = not yet known, binds nothing); never filled by a skill on its own
```
...
(status: planned | skeleton | drafted | polished);
...
`SIM_REVIEW_<date>.md` is the peer-review panel's meta-review, venue-shaped. Frontmatter: `type: peer_review`, `target:`, `cycle:`, `scale:` (conference-6 | journal), `mode:` (panel | quick), `generated:`, `commit:` (the reviewed `git log -1` sha, `+dirty` when `manus/` had uncommitted edits), `recommendation:`. The same skill's `extern=` run writes the same sections to a `REFEREE_<date>.md` under `wkdrs/` or the `out=` directory the user named, with `type: referee_report`, a `venue:` the user confirmed, and no `cycle:` or `commit:` — it is deliberately outside this table, because nothing downstream reads it. Sections: `## Summary`, `## Strengths`, `## Major Weaknesses` (numbered; each anchored, naming the claim IDs it attacks and the perspectives that raised it), `## Minor Weaknesses`, `## Questions to the Authors`, `## Limitations & Ethics`, `## Concern Matrix`, `## Recommendation` (anchored band or journal tier, confidence, every triggered cap named), `## Synthesis Notes`. The full templates, the five perspective briefs, and the rubrics live in `stage-peer-reviewer`'s `references/`; per-perspective reviews and the citation audit stay in the run's `wkdrs/reports/peer_<cycle>_<date>/` directory.
`RESPONSE_<date>.md` frontmatter: `cycle:`, `date:`, `sources:` (the review files `## Draft response` answers). Sections: `## Point ledger` `| Point | Reviewer | Attacked claims | Evidence | Response summary | Promise? |`, `## Draft response` (within `response_limit`), and promises mirrored to `tasks/<cycle>_promises.md` as `- [ ]` checkboxes; a box withdrawn from the response moves under `## Withdrawn` without checkbox syntax.

### 8.9 `notes/adopt.md`

Frontmatter: `adopted:`, `backfilled:`. Sections: paired sources (STAR repos + slugs), venue target, existing-asset inventory (for adopted projects), the unsourced-claims backlog.

`backfilled:` is a gate, not a note (§9a). It stays empty while any backlog row is unresolved, and it is set — to the real date — only by `stage-clms-auditor`, when every row has become a `verified` claim, an `unsourced` one carrying its `\todo`, a claim about a cited work handed to `stage-cite-auditor`, or a value the typeset text no longer states because its sentence was rewritten not to need it (§9a). A row is found by its `file:line`, and by its recorded claim text where a later move or renumbering left that line stale. `stage-subm-packer` refuses to pack a repository that has a `notes/adopt.md` with an empty `backfilled:`, because that is exactly the state in which `lint.sh` reads clean over numbers that trace to nothing.

### 8.10 `cycls/<cycle>/SUBMISSION_<date>.md`

Frontmatter: `cycle:`, `date:`, `frozen:` (tag name), `package:` (path under `wkdrs/builds/`), `template:` (the venue template the package was formatted in, or `arxiv`). Body: lint summary, checklist outcome, the claims the latest audit did not verify with their waivers, the open audit follow-ups with their waivers, page counts — the converted copy's, with the preprint build's beside it when they differ — what the conversion dropped or left for a human, and what was submitted where.
...
**It binds `manus/` prose and nothing else.** Not the Markdown a run writes under `notes/`, `tasks/`, or `wkdrs/` — those are records with schemas of their own — and not `cycls/<cycle>/response/`, whose register is fixed by the venue's `response_type` and `response_limit`. The file is optional: absent means no profile applies, and no skill creates one as a side effect of another job.

### 8.12 `tasks/*_followups.md`

The three audit follow-up lists — `tasks/claims_followups.md`, `tasks/cites_followups.md`, `tasks/polish_followups.md` — hold one `- [ ]` per finding under a `## <date>` heading, each written by the skill that owns it (§8, Audit reports). A re-run first checks off what it proves resolved, then appends only findings with no open box at the same location and verdict, under today's heading, reused rather than repeated on a same-day re-run (§4.4). Unlike the venue list (§8.10), a finding checked off earlier that fails again is filed again: an audit re-raises a regression.
...
- **A number a sentence attributes to a cited work is that work's fact.** The value a `\cite` in the same sentence attaches to — "ODISE reaches 23.4 PQ on ADE20K" — is backed by that work's reading note under (b) and audited by `stage-cite-auditor`; `stage-clms-auditor` hands it off rather than walking the prose chain. A table row stating it still carries its `mates/` `% src:` like every data row. This paper's own numbers in the same sentence stay under this rule, and so does a delta against the cited value, which is a derived number (above).
...
- `confirmed:` is the receipt: the date the **user** confirmed the numbers, never filled by a skill on its own (§8.3). A profile with empty `confirmed:` gates nothing — `stage-outl-planner` will not budget against its page limit and `stage-subm-packer` will not pack against it without asking first. A blank value binds nothing either, `confirmed:` set or not: `stage-outl-planner` needs `page_limit_main`, and `stage-subm-packer` needs `page_limit_main`, `anonymized:`, and `checklist:`, each treating its blank value as it treats an empty `confirmed:`. A wrong page budget discovered at pack time costs a rewrite; that is why this is a mandatory confirmation point (§7.7).

**(d) Evidence files are immutable in place.**

- The only writers under `mates/` are `execs/scpts/import.sh` and `stage-evid-curator`, and they only add or replace whole files with fingerprint updates. No in-place edit exists — not a typo, not a unit conversion, not "just this one cell". A wrong number upstream is fixed upstream (in STAR, or by the human who produced the drop), then re-imported; the fingerprint moves, `stage-clms-auditor` re-verdicts every table row and sentence that cited it, and `stage-figs-designer`'s no-argument audit demotes every figure whose source recorded the old `sha256:`.
...
1. **`mates/` is read-only** — §9d names its two writers and how they write.
2. **`wkdrs/` is never committed** (§1.2).
...
5. **Seven harnesses share one neutral skill store.** The same sixteen skills ship in `.agents/skills/`,
   ...
   names roles rather than another harness's tools, apart from `stage-figs-designer`'s optional local figure pipeline, which its Step 0 takes only where those tools exist. Codex's per-skill UI manifests live under
...
| `stage-evid-curator` | exec | import and register evidence |
...
2. **Two skills only report.** `stage-peer-reviewer` writes only its review under `cycls/<cycle>/reviews/` — or, refereeing an external paper with `extern=`, only under `wkdrs/` or the `out=` directory the user named outside `manus/`, `notes/`, `cycls/`, `mates/`, and `tasks/`; `stage-flow-status` writes nothing at all. Run the status skill first whenever you do not know where things stand — it reads the outline, the ledger, the manifest, and the cycle state, and names the single next action with its exact command.
...
4. **A run ends with its report and the exact next command; it does not start the next run.** Skills end by naming what comes next — the status skill's single next action, a red lint gate naming the owner of what broke, an auditor routing an unsupported claim to the section that carries it — and each is printed as the exact command (§7.5), even when it names one of the ten. Continuing is the job of `stage-auto` (item 5): typed with a goal, it takes the named action itself, one of the ten at a time, and stops with the command wherever the next step is the author's. Outside it, a successor starts only when the user's own request asked for it — "draft 3_method, then audit its numbers" (§7.13) — and then under three limits. **It starts after the naming run has ended, never inside it** — a skill that may not touch the manuscript gains no reach by naming a successor. **An unsettled target is asked about rather than guessed** — which section, which table, which figure, which cycle (§5 resolves them). And **item 3 holds unchanged**, one skill and one unit of work, with a run nobody typed saying what it is starting before it begins. **A read-only request never starts a writing skill**: a request for status, an audit, or a review — to be told, not to have the paper changed — authorizes that deliverable alone; a successor gains no permission by appearing in its report, and neither `INVOLVE=low` nor a recommendation it takes (§7.7) is the user asking. The six keep the printed command in every case, because typing it *is* the decision they exist to leave with the author.
5. **`stage-auto` is the goal-run grant.** `stage-auto <goal> [involve=<level>]` is a command, not a skill — `/stage-auto` in Claude Code, Cursor, DSH, Pi, and Qwen Code, `$stage-auto` in Codex, `/stage-auto` (`/skill:stage-auto`) in Kimi Code — and typing it is the one request that authorizes pursuing a goal across runs. Its procedure, [`.agents/commands/stage-auto.md`](../../../.agents/commands/stage-auto.md), turns the goal into a check on repository state — a goal that only an action outside the grant could meet gets that action's command and no run at all — runs `stage-flow-status` first, and stops at the first of: the check passing, a † skill, a mode that exists to take the author's choice, a STOP-line action, a question nobody can answer, a run that would write a path an earlier run of the same invocation left uncommitted, a check resting on a venue value nobody confirmed, nothing left but work only the author can unblock — evidence `mates/` does not hold, a promise the author has not confirmed kept — a repeat of the same start, a full pass that moved nothing, or a failed action whose routed fix failed too. Item 4 ends every run at its report and exact next command; typing `stage-auto` is what takes that command up: the goal run takes each named next action, never the run that named it, and only through the goal filter, so a named action that does not advance the goal is listed rather than taken, and the goal run starts instead the unmarked skill that owns the goal's next step, or the owner of that step's first missing input; each start is one of the ten, one unit of work (item 3), at the goal run's `involve=` level — raised by a printed command that asks more, never lowered by one — and behaves exactly as if the user had typed it, its own commit step (§1) included. A goal that asks only to be told — where the paper stands, an audit, a review — is a read-only request (item 4): the goal run starts only the skill that produces that report, and where the next step would need any other — the owner of a missing input, a fix the report routes — it stops and prints that command. It never starts one of the six marked †: the run stops there and prints the exact command. It never writes `mates/`, enters a venue fact (§9c), crosses the STOP line (§2), answers a mandatory confirmation point (§7.7), or commits, tags, or writes anything itself, and green lint is never its goal.
