...
The four variables, as `.env.example` ships them:
...
1. **`.env` at the repository root is the only source** of these values. Never guess a local path, never hardcode one, never read them from memory of another project. `.env` itself is git-ignored and machine-specific.
2. **Every variable has a working default**, so a missing `.env` never blocks a build: `LATEX_ENGINE` falls back to pdflatex, `ANON` to false. Empty `STAR_HOME` is a supported state — writing without a paired repository — in which `import.sh` requires `--source` and evidence arrives as manual drops. A skill that needs `STAR_HOME` and finds none asks (§7); it never invents a path.
...
6. **Reply in the user's dialogue language.** A Chinese conversation gets Chinese replies. Everything the workflow writes into the repository is English in v1 — notes, ledger rows, reviews, reports; there is no language override variable yet (roadmap). Inside Chinese replies keep technical terms, metric names, venue names, file paths, and everything from `reference.bib` in English. The manuscript itself sits outside this rule: its language belongs to the venue and the user, and the chat's language never rewrites it.
