...
**The discipline is one commit per skill working session** — never unannounced, staging only that session's work, with the subject prefix naming the skill: `stage-sect-drafter: 3_method — first full draft`, `stage-evid-curator: import xseg results`. One skill, one namespace in the log.
