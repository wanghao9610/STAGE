...
`backfilled:` is a gate, not a note (§9a). It stays empty while any backlog row is unresolved, and it is set — to the real date — only by `stage-clms-auditor`, when every row has become a `verified` claim, an `unsourced` one carrying its `\todo`, or a claim about a cited work handed to `stage-cite-auditor`. `stage-subm-packer` refuses to pack a repository that has a `notes/adopt.md` with an empty `backfilled:`, because that is exactly the state in which `lint.sh` reads clean over numbers that trace to nothing.
