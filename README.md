...
- **zh-CN mirrors for human readers**: `*_zh.md` beside skills' `references/` files, `writing-workflow-skills.zh-CN.md` beside the skills guide, and the Chinese landing pages under `docs/` — kept in step, never loaded at runtime, and the English files stay authoritative.
...
│   ├── hooks/              # Hooks: project-memory index, session model id, involve and bash gates, commit guard
...
**Two session hooks.** One supplies the [project memory](#project-memory) index; the other states the runtime's model id so artifacts record `model_id` and `model_trail` (workflow conventions §8). In Claude Code both also run when a sub-agent starts, since a session hook does not fire for one: the provenance hook hands the delegate the command that resolves the delegate's own transcript, so an artifact a delegate writes records the model that wrote it rather than the session's, and the memory hook repeats the index for it. Claude, Codex, Cursor, Pi, and Qwen register them from the project. Kimi and DSH need their one-time installers (`.kimi-code/hooks/install.sh`, `.dsh/hooks/install.sh`) because those harnesses keep hook registration outside the repository. Codex project hooks still require approval through `/hooks`.

**Three more hooks decide rather than inject.** Claude, Codex, and Qwen carry the `INVOLVE=low` edit-permission gate. Claude also carries its shell counterpart, `stage_bash_gate.sh`: at `low` it allows ordinary local commands — builds, lint, greps, scripts, staging and committing by name — while deletion, an overwrite of a tracked file, any write under `mates/` except through `execs/scpts/import.sh`, anything but a read of a venue kit or `.env`, installs (`sudo`, `tlmgr`, package managers), `git push` and other outward sends, `git pull`, job launches, history rewrites and pruning, branch switches, tag operations other than listing, and process or system control keep the host's normal prompt; the commit guard's deny still outranks its allow. Every harness carries `stage_commit_guard.sh`, which declines the git commands [conventions §1](docs/mds/stage-workflow/writing-workflow-conventions.md) forbids. Pi registers it in its project extension; DSH and Kimi use their hook bridges.
...
<div align="center">
  <img src="docs/mds/stage-workflow/stage-writing-workflow.png" alt="STAGE writing workflow: fifteen skills in the order they run in plus one that reads across them, what each one writes, and how the drafting loop and the rejection loop close" width="100%">
</div>
...
- `docs/mds/stage-workflow/` — the workflow conventions (project memory is its §12, harness hooks and model provenance its §13), the skill guide and its Chinese edition, and the workflow diagram the guide embeds
...
Harness configuration — `.cursorignore`, `.claude/settings.json`, `.codex/hooks.json`, `.cursor/hooks.json`, `.pi/settings.json`, and `.qwen/settings.json` — and the line-break rule `fmt.sh` and `lint.sh` read, `.latexindent.yaml` with its editor half `.editorconfig`, are installed only when missing and otherwise kept unless `--force` is supplied. The command reports a kept file that differs from upstream and names any STAGE hook absent from a kept registration.

A `.claude/settings.json` kept from before the sub-agent hooks lacks two entries, and the command names each in a note: the `SubagentStart` block (reported as the missing `SubagentStart delegate context` hook) and the allow rule for the model-id resolver, without which a delegate records its `model_id` as `unrecorded` because it cannot answer a permission prompt. Copy both from upstream's file into yours: the block's two hook commands, and `"Bash(bash .claude/hooks/stage_model_id.sh --resolve:*)"` under `permissions.allow`.
...
Files at matching paths are overwritten and new upstream files are added. Project-specific files that exist only in the updated directories are preserved. To avoid deleting custom content, files removed upstream are not removed locally. STAGE no longer ships the following files, and an update keeps any copy your repository already has, so delete each one that came from STAGE: `AGENTS.zh-CN.md` and `CLAUDE.zh-CN.md`; the `SKILL_zh.md` beside each skill and router plugin; the `stage.zh-CN.md` router in `.agents/commands/` and its four native wrappers in `.claude/commands/`, `.cursor/commands/`, `.pi/prompts/`, and `.qwen/commands/`, each of which adds a duplicate `/stage.zh-CN` entry to its harness's command menu; and, in `docs/mds/stage-workflow/`, every `*.zh-CN.md` twin except the skill guide's, together with `human-writing-guide.md`, `memory_spec.md`, and `model_id_spec.md`, whose rules the conventions now carry in §7, §12, and §13. A paper set up before the memory hooks built their index from each memory's frontmatter keeps its `.stage/memory/MEMORY.md` and `MEMORY.zh-CN.md`: no hook reads either any more, and you may delete them. The update does not modify other directories, the current branch, Git remotes, or the staging area — the manuscript, `mates/`, `notes/`, and `cycls/` are never in scope. Commit current work before updating, then review and commit the result with `git status` and `git diff`.

Working on STAGE itself rather than on a paper? Edit only the neutral authored source under `.agents/skills/`, then run `bash .github/scripts/port.sh --write` to regenerate all six harness trees. Harness-only behavior belongs in that tree's rules or anchored overrides. Run `bash .github/scripts/port.sh` to prove every generated harness and shared link still matches, then `bash .github/scripts/check_consistency.sh` for the seven-root semantic invariants. The two checks run in CI. STAGE's own memories go under the git-ignored `.stage/memory/local/` whatever their scope: a clone or the GitHub template copies the versioned `.stage/memory/` as is, so a memory about developing STAGE would reach every paper as a fact about that paper. `check_consistency.sh` fails on any tracked file there beyond the `.gitkeep` the template ships.
...
8. What a session learns that no file above owns goes to `.stage/memory/`, offered before it is written, with machine-specific facts and anything you keep off the repository in the git-ignored `.stage/memory/local/`; a memory never sources a number, a venue rule, or a claim about a cited paper.
