...
| `stage-peer-reviewer` | Simulated review, one lens per run (novelty, rigor, clarity, related-work), rubric from `venue.yml`, output in the same format as a real review; never edits the manuscript | `cycls/<cycle>/reviews/SIM_REVIEW_<lens>_<date>.md` |
...
9. **Review and respond** — `/stage-peer-reviewer` simulates one review lens per run into `cycls/<cycle>/reviews/`; real reviews are dropped there as `received_<id>.md`; `/stage-resp-writer` turns them all into a point ledger, a response within the venue's limit, and promise checkboxes in `tasks/`.
